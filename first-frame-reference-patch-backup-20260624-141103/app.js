...
function updateProductionTopBanner() {
    ...
    if (videoMeta) videoMeta.textContent = runningVideo ? '视频生成中' : (total ? `${videos}/${total} 视频` : '等待 Video Prompt');
    ...
    const stepStates = {
        ...
        video: total && videos === total ? 'done' : (runningVideo ? 'running' : (total && videoPrompts ? 'running' : 'blocked'))
    };
...
function buildMainlineVideoGenerationPrompt(seg, index, imagePrompt, manualVideoPrompt = '') {
    ...
    const storyboardLine = seg?.storyboard_image_path
        ? '已随本次请求附带当前片段分镜图；严格按分镜图锁定构图、人物左右关系、镜头高度、动作方向和节奏。'

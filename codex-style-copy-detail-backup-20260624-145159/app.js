...
function getVideoPromptStatus(seg) {
    ...
    if (getSegmentVideoPrompt(seg)) return renderPipelineStatusPill('ready', 'Video Prompt 就绪');
...
function renderFlowContext(stage, segments = []) {
    ...
    const config = {
        script: {
            eyebrow: 'STEP 01',
            title: '台本分段 + Image Prompt + 分镜图',
            input: '输入：完整台本 / 素材',
            output: '输出：片段原台词 / Image Prompt / 分镜图',
            action: '下一步：分镜图 → Video Prompt'
        },
        image: {
            eyebrow: 'STEP 02',
            title: '分镜图 → Video Prompt',
            input: '输入：Image Prompt / 分镜图',
            output: '输出：单段 Video Prompt',
            action: '下一步：Video Prompt → 视频'
        },
        video: {
            eyebrow: 'STEP 03',
            title: 'Video Prompt → 视频',
            input: '输入：分镜图 / Video Prompt',
            output: '输出：当前片段视频',
            action: '完成后可继续下一片段或进入视频编辑'
        }
    }[stage] || {};
    return `
        <div class="flow-context-card flow-context-${escAttr(stage)}">
            ...
            <div class="flow-context-track" aria-label="当前片段流程状态">
                ${renderFlowStepNode('01', '台本 / Image / 分镜', step1Status, stage === 'script')}
                ${renderFlowStepNode('02', 'Video Prompt', step2Status, stage === 'image')}
                ${renderFlowStepNode('03', '生成视频', step3Status, stage === 'video')}
            </div>
        </div>`;
}
...
function renderFlowSidebarChecklist(seg = {}) {
    ...
    const items = [
        ['原台词', state.source],
        ['Image Prompt', state.imagePrompt],
        ['分镜图', state.storyboardImage],
        ['Video Prompt', state.videoPrompt],
        ['视频', state.video]
    ];
    return `
...
function renderScriptPromptOverview(segments) {
    ...
    const activeStatus = activePrompt
        ? renderPipelineStatusPill('ready', 'Prompt 已生成')
        : (activeSeg.storyboard_status === 'generating'
            ? renderPipelineStatusPill('running', '生成中')
            : renderPipelineStatusPill('idle', '等待生成'));
    const activeImagePreview = activeImagePath
        ...
        : `<div class="storyboard-image-empty">
                <strong>${activePrompt ? '等待生成分镜图' : '等待 Image Prompt'}</strong>
                <span>${activePrompt ? '点击生成分镜图后，图片会显示在这里。' : '先生成或编辑当前片段 Image Prompt。'}</span>
           </div>`;
    const activeStoryboardEditTools = activeImagePath ? `
        <div class="storyboard-image-edit-tools">
            <div class="storyboard-image-edit-path">当前分镜图：${escHtml(activeImagePath)}。生成 Video Prompt 和视频时会作为核心参考图输入。</div>
            <label for="storyboardEditPrompt-${activeIndex}">编辑提示词</label>
            <textarea class="storyboard-prompt-editor storyboard-image-edit-editor" id="storyboardEditPrompt-${activeIndex}" rows="5" placeholder="输入要修改的内容，例如：第3格改成近景眼神特写；保持其他格子不变" oninput="currentSegments[${activeIndex}]._storyboardEditPrompt = this.value">${escHtml(activeSeg._storyboardEditPrompt || '')}</textarea>
            <div class="storyboard-image-edit-actions">
                ...
                <a class="btn btn-sm btn-secondary" href="/${escAttr(activeImagePath)}" target="_blank">查看大图</a>
    ...
    const renderCard = (seg, i) => {
        ...
        const imageStatus = imagePath
            ...
            : (seg.storyboard_status === 'generating'
                ...
                : renderPipelineStatusPill('idle', prompt ? '可出图' : '等 Prompt'));
        ...
        return `
            <article class="script-prompt-card ${i === activeIndex ? 'active' : ''}" onclick="selectScriptPromptSegment(${i})">
                ...
                <div class="script-prompt-meta">
                    <span>${duration}秒</span>
                    <span>${prompt ? `${prompt.length}字 Prompt` : '暂无 Prompt'}</span>
    ...
    return `
        <div class="script-prompt-page">
            ...
            <div class="script-prompt-page-head">
                <div>
                    <span class="eyebrow">合并流程</span>
                    <h3>台本分段 / Image Prompt / 分镜图</h3>
                    <p>片段 ${activeIndex + 1} · ${activeSeg.duration || 15}秒 · ${activePrompt ? 'Image Prompt 已生成' : '等待 Image Prompt'}</p>
                </div>
                <div class="script-prompt-page-actions">
                    <span class="badge">${ready}/${segments.length} Prompt</span>
                    <span class="badge">${imageReady}/${segments.length} 分镜图</span>
                    <button type="button" class="btn btn-sm btn-primary script-prompt-generate-all" onclick="generateAllStoryboardPrompts()">全部生成 Image Prompt</button>
                    <button type="button" class="btn btn-sm btn-secondary" onclick="generateAllStoryboardImages()" ${ready ? '' : 'disabled'}>全部生成分镜图</button>
                </div>
            </div>
            ${generating ? '<div class="script-prompt-loading">正在生成或更新 Prompt/分镜图，卡片会逐段刷新。</div>' : ''}
            <section class="script-prompt-feature">
                <div class="script-prompt-feature-head">
                    <div>
                        ...
                        <p>${activeSeg.duration || 15}秒 · 原台词 → Image Prompt → 分镜图</p>
                    </div>
                    <div class="script-prompt-feature-actions">
                        ${activeStatus}
                        ${getStoryboardImageStatus(activeSeg)}
                        <button type="button" class="btn btn-sm btn-secondary" onclick="copySegmentImagePrompt(${activeIndex})" ${activePrompt ? '' : 'disabled'}>复制 Prompt</button>
                        <button type="button" class="btn btn-sm btn-secondary" onclick="generateStoryboardPrompt(${activeIndex})">重新生成 Prompt</button>
                        <button type="button" class="btn btn-sm btn-primary" onclick="regenerateStoryboardImage(${activeIndex})" ${activePrompt || activeSeg.id ? '' : 'disabled'}>${activeImagePath ? '重新生成分镜图' : '生成分镜图'}</button>
                    </div>
                </div>
                <div class="step-transfer-strip">
                    <span class="done">原台词分段</span>
                    <i></i>
                    <span class="${activePrompt ? 'done' : 'active'}">Image Prompt</span>
                    <i></i>
                    <span class="${activeImagePath ? 'done' : (activePrompt ? 'active' : '')}">分镜图</span>
                    <i></i>
                ...
                <div class="script-prompt-feature-body script-prompt-merged-body">
                    <div class="script-prompt-merged-left">
                        ...
                        <div class="script-prompt-editor-wrap">
                            ...
                            <textarea class="script-prompt-large-editor" id="storyboardPrompt-${activeIndex}" rows="18" placeholder="这里显示当前片段的 Image Prompt。可直接手动编辑，失焦后自动保存，再生成分镜图。" onblur="saveStoryboardPromptEdit(${activeIndex})">${escHtml(activePrompt)}</textarea>
                        </div>
                    </div>
...
function renderVideoPromptSidebarItem(seg, i, activeIndex) {
    ...
    const statusText = failed ? '失败' : (running ? '生成中' : (promptReady ? 'Video Prompt' : (imageReady ? '可生成' : '等分镜图')));
    const thumb = imageReady
        ? `<img src="/${escAttr(seg.storyboard_image_path)}" alt="分段 ${i + 1}">`
        : `<span>${i + 1}</span>`;
    return `
            ...
            <section class="storyboard-image-main">
                ...
                <div class="storyboard-image-main-head">
                    <div>
                        <span class="eyebrow">分段 ${activeIndex + 1}</span>
                        <h3>${escHtml(title)}</h3>
                        <p>Image Prompt + 分镜图 → 单个 15 秒 Video Prompt</p>
                    </div>
                    <div class="storyboard-image-head-actions">
                        ${getVideoPromptStatus(seg)}
                        <button type="button" class="btn btn-sm btn-secondary" onclick="generateAllVideoPrompts()" ${segments.length ? '' : 'disabled'}>全部生成 Video Prompt</button>
                        <button type="button" class="btn btn-sm btn-primary" onclick="generateVideoPrompt(${activeIndex})" ${canGenerate && !loading ? '' : 'disabled'}>${videoPrompt ? '重新生成 Video Prompt' : '生成 Video Prompt'}</button>
                    </div>
                </div>
                <div class="video-prompt-workspace">
                    <div class="video-prompt-input-panel">
                        <div class="storyboard-image-panel-head">
                            <strong>输入：Image Prompt</strong>
                            <span>${imagePrompt ? `${imagePrompt.length}字` : '未生成'}</span>
                        </div>
                        <textarea class="storyboard-image-prompt-editor video-prompt-input-editor" id="storyboardPrompt-${activeIndex}" rows="12" placeholder="这里显示第 1 页生成的分段 Image Prompt；可手动微调，失焦保存。" onblur="saveStoryboardPromptEdit(${activeIndex})">${escHtml(imagePrompt)}</textarea>
                    ...
                    <div class="video-prompt-output-panel ${loading ? 'is-loading' : ''}">
                        <div class="storyboard-image-panel-head">
                            <strong>输出：Video Prompt</strong>
                            <span>${videoPrompt ? `${videoPrompt.length}字` : '未生成'}</span>
                        </div>
                        ${loading ? `
                            <div class="storyboard-image-loading">
                                <span class="loading-spinner"></span>
                                <div><strong>正在生成 Video Prompt</strong><small>只使用当前片段的 Image Prompt 和分镜图，不重新输入原台本。</small></div>
                            </div>` : ''}
                        <textarea class="storyboard-image-prompt-editor video-prompt-output-editor" id="videoPrompt-${activeIndex}" rows="24" placeholder="点击“生成 Video Prompt”后显示；也可以手动编辑，失焦保存。" onblur="saveVideoPromptEdit(${activeIndex})">${escHtml(videoPrompt)}</textarea>
                        <div class="video-prompt-actions">
                            ...
                            <button type="button" class="btn btn-sm btn-secondary" onclick="copyVideoPrompt(${activeIndex})" ${videoPrompt ? '' : 'disabled'}>复制 Video Prompt</button>
                            <button type="button" class="btn btn-sm btn-primary" onclick="focusProductionFlowStep('video')" ${videoPrompt ? '' : 'disabled'}>进入生成视频页</button>
                        </div>
                    </div>
                </div>
            </section>
...
function renderVideoGenerationSidebarItem(seg, i, activeIndex) {
    ...
    const statusText = failed ? '失败' : (running ? '生成中' : (ready ? '视频完成' : (getSegmentVideoPrompt(seg) ? '可生成' : '等 Prompt')));
...
function renderVideoGenerationPage(segments) {
    ...
    const imagePreview = imagePath
        ...
        : `<div class="storyboard-image-empty">
                <strong>等待分镜图</strong>
                <span>先在第 01 页生成分镜图，再在第 02 页生成 Video Prompt。</span>
           </div>`;
    const videoResult = videoSrc
        ...
        : `<div class="storyboard-image-empty video-generation-empty">
                <strong>${segStatus === 'generating' ? '视频生成中' : '等待生成视频'}</strong>
                <span>${videoPrompt ? '点击生成后，视频结果会显示在这里。' : '先到第 02 页生成 Video Prompt。'}</span>
           </div>`;
    return `
            ...
            <section class="storyboard-image-main">
                ...
                <div class="video-prompt-workspace video-generation-workspace">
                    <div class="video-prompt-input-panel">
                        ...
                        <div class="storyboard-image-panel-head video-prompt-image-head">
                            <strong>输入：Video Prompt</strong>
                            <span>${videoPrompt ? `${videoPrompt.length}字` : '未生成'}</span>
                        </div>
                        <div class="segment-editor mention-editor stage-video-editor video-generation-prompt-editor" id="segEditor-${activeIndex}" data-rows="14" data-placeholder="这里显示第 02 页生成的 Video Prompt；可手动微调。"></div>
                        ...
                        <details class="stage-inline-details video-generation-settings">
                            <summary>视频参数 / 参考图</summary>
                            <div class="segment-mode-row">
                                <label>生成模式</label>
                                <select id="segMode-${activeIndex}" class="seg-mode-select" onchange="onSegModeChange(${activeIndex})">
                                ...
                                <label>比例</label>
                                <select id="segRatio-${activeIndex}" class="seg-small-select">
                                    ${['16:9','9:16','1:1','4:3','3:4','21:9','adaptive'].map(r => `<option value="${r}" ${ratio === r ? 'selected' : ''}>${r}</option>`).join('')}
                                </select>
                                <label>时长</label>
                                <input type="number" id="segDuration-${activeIndex}" min="4" max="15" value="${duration}" class="seg-duration-input">
                            </div>
                            <div id="segRefWrap-${activeIndex}" class="${mode==='text_to_video'?'hidden':''}">
                                <div class="segment-upload-row">
                                    <span>临时上传参考图：</span>
                                    <input type="file" id="segFile-${activeIndex}" accept="image/*" multiple onchange="onSegmentFilesChange(${activeIndex})">
                                    <span id="segFileCount-${activeIndex}" class="segment-file-count">${seg._uploadFileCount || 0}张</span>
                                </div>
                            </div>
                        </details>
                    </div>
                    <div class="video-prompt-output-panel ${segStatus === 'generating' ? 'is-loading' : ''}">
                        ...
                        <div class="video-prompt-actions">
                            ...
                            <button type="button" class="btn btn-sm btn-secondary" onclick="focusProductionFlowStep('image')">返回 Video Prompt</button>
                        </div>
                        <details class="video-generation-editor-panel" ${videoSrc ? 'open' : ''}>
                            <summary>视频编辑 / 局部替换</summary>
                            <div class="video-generation-editor-grid">
                                <div class="video-generation-editor-input">
                                    <div class="storyboard-image-panel-head">
                                        <strong>编辑源视频</strong>
                                        <span>${videoEditorSourcePath ? '已载入' : '未载入'}</span>
                                    </div>
                                    <div class="video-editor-inline-actions">
                                        <button type="button" class="btn btn-sm btn-secondary" onclick="loadSegmentVideoToEditor(${activeIndex})" ${videoSrc ? '' : 'disabled'}>载入当前片段视频</button>
                                        <button type="button" class="btn btn-sm btn-secondary" onclick="importMergedVideoToEditor()">导入当前项目拼接成片</button>
                                    </div>
                                    <div class="form-group">
                                        <label>上传待编辑视频</label>
                                        <input type="file" id="videoEditorUpload" accept="video/*">
                                    </div>
                                    <div class="video-editor-inline-actions">
                                        <button type="button" class="btn btn-sm btn-primary" onclick="uploadVideoEditorSource()">上传并载入</button>
                                    </div>
                                    <div id="videoEditorSourceInfo" class="local-path">尚未载入视频</div>
                                    <div class="video-range-repair-grid">
                                        <label>开始秒<input type="number" id="videoEditorStart" min="0" step="0.1" placeholder="例如 4.0"></label>
                                        <label>结束秒<input type="number" id="videoEditorEnd" min="0" step="0.1" placeholder="例如 7.0"></label>
                                    </div>
                                    <div id="videoEditorPrompt" class="segment-editor mention-editor video-editor-inline-prompt" data-rows="6" data-placeholder="描述这段小片段需要如何修改，输入 @ 引用素材或提示词库；例如：人物表情更惊讶、镜头更近、动作改为转身后退。"></div>
                                    <div class="mention-preview-row">
                                        <button type="button" class="btn btn-sm btn-secondary mention-insert-btn" onclick="openMentionPickerForVideoEditor()">@ 插入</button>
                                        <span class="mention-preview-hint">素材会作为这次视频编辑参考图，提示词库会作为附加修改要求。</span>
                                    </div>
                                    <div class="form-group" style="margin-top:12px;">
                                        <label>额外上传临时参考图（可选）</label>
                                        <input type="file" id="videoEditorReferenceFiles" accept="image/*" multiple onchange="onVideoEditorReferenceFilesChange()">
                                        <div id="videoEditorReferenceFileCount" class="field-hint">0张 · 临时上传的参考图会和 @ 到的素材一起发给视频模型</div>
                                    </div>
                                    <button type="button" class="btn btn-primary btn-lg" onclick="submitStandaloneVideoEdit()">提交视频编辑</button>
                                    <div id="videoEditorStatus" class="local-path"></div>
                                </div>
                                <div class="video-generation-editor-preview">
                                    <div id="videoEditorPreview" class="video-editor-preview empty-state"><p>载入或上传视频后将在这里预览</p></div>
                                    <div id="videoEditorUndoPanel" class="video-editor-undo-panel hidden"></div>
                                    <div id="videoEditorPendingResult" class="video-editor-confirm-panel hidden"></div>
                                </div>
                            </div>
                        </details>
                    </div>
...
function updateProductionTopBanner() {
    ...
    if (scriptMeta) scriptMeta.textContent = runningPrompt
        ? 'Prompt / 分镜图生成中'
        : (total ? `${total} 段，${prompts}/${total} Prompt，${images}/${total} 分镜图` : (scriptText ? '台本已输入' : '等待台本'));
    if (imageMeta) imageMeta.textContent = total ? `${videoPrompts}/${total} Video Prompt` : '等待分镜图';
    if (videoMeta) videoMeta.textContent = runningVideo
        ? '视频生成中'
        : (total ? `${videoPrompts}/${total} Video Prompt，${videos}/${total} 视频` : '等待 Video Prompt');
...
async function generateVideoPrompt(index, options = {}) {
    ...
    try {
        ...
        if (resultEl) resultEl.textContent = currentSegments.map(s => getSegmentVideoPrompt(s) || s.prompt || s.prompt_text || '').join('\n\n===VIDEO_SPLIT===\n\n');
        renderSegments(currentSegments);
        if (!options.silentLoading) showToast(`片段 ${index + 1} Video Prompt 已生成`, 'success');
        return true;
    } catch (e) {
        seg._videoPromptStatus = 'failed';
        renderSegments(currentSegments);
...
function renderVideoEditStudioPage(segments) {
    ...
    const preview = videoSrc
        ...
        : (imagePath ? `<img src="/${escAttr(imagePath)}" alt="当前分镜图">` : `<div class="edit-studio-empty"><strong>暂无预览</strong><span>生成视频后显示成片；当前可先按场景检查时间线与参数。</span></div>`);
    return `
        <div class="edit-studio-page">
            ...
            <section class="edit-studio-main">
                <div class="edit-studio-project-bar">
                    <div><strong>${escHtml(projectTitle)}</strong><small>当前场景 ${activeIndex + 1} / ${segments.length}</small></div>
                    <div><button class="btn btn-sm btn-secondary" onclick="mergeProjectVideos()">导出成片</button><button class="btn btn-sm btn-primary" onclick="mergeProjectVideos()">发布成片</button></div>
                </div>
                ...
                <div class="edit-studio-timeline">
                    ...
                    <div class="edit-studio-tools"><button title="按播放头分割">✂ 分割</button><button title="删除当前片段">⌫ 删除</button><button title="复制当前片段">＋ 复制</button><button title="撤销上一步">↶ 撤销</button></div>
                </div>
            </section>
            <aside class="edit-studio-settings">
                ...
                <label>当前提示词<textarea rows="12" readonly>${escHtml(promptText || '暂无 Video Prompt；请先在 02「分镜 / 视频」生成。')}</textarea></label>
                <div class="edit-studio-meta"><strong>${escHtml(title)}</strong><span>当前场景 ${duration}s · 全片约 ${totalDuration}s</span></div>
            </aside>
        </div>`;
}

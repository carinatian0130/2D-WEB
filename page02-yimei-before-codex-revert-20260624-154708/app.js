...
function renderVideoGenerationPage(segments) {
    ...
    const sceneItems = segments.map((item, i) => {
        normalizeSegmentState(item);
        const itemTitle = getSegmentStoryTitle(item, i);
        const itemReady = !!(item?._localVideoPath || item?._videoUrl || item?.video_url || item?.local_video_path || item?.status === 'completed');
        const itemPrompt = !!getSegmentVideoPrompt(item);
        const itemImage = !!item.storyboard_image_path;
        const stateText = itemReady ? '已完成' : (itemPrompt ? '可生成' : (itemImage ? '待提示' : '待分镜'));
        const stateClass = itemReady ? 'ready' : (itemPrompt ? 'active' : 'wait');
        const thumb = item.storyboard_image_path ? `<img src="/${escAttr(item.storyboard_image_path)}" alt="场景${i + 1}">` : `<span>${i + 1}</span>`;
        return `
            <button type="button" class="page02-account-item ${i === activeIndex ? 'selected' : ''}" onclick="activeSegmentEditIndex=${i}; activeProductionFlowStep='video'; renderSegments(currentSegments);">
                <span class="page02-account-thumb">${thumb}</span>
                <span class="page02-account-copy"><strong>场景${i + 1}</strong><small>${escHtml(itemTitle)}</small></span>
                <em class="${stateClass}">${stateText}</em>
            </button>`;
    }).join('');
    const coverPreview = imagePath ? `<img src="/${escAttr(imagePath)}" alt="当前分镜图">` : `<span>+</span>`;
    const videoPreview = videoSrc
        ? `<video src="${escAttr(videoSrc)}" controls preload="none"></video>`
        : `<div class="page02-empty-preview"><strong>${segStatus === 'generating' ? '生成中' : '待生成'}</strong><span>${videoPrompt ? '点击右侧立即生成' : '先生成视频提示'}</span></div>`;
    return `
        <div class="page02-distribute-editor video-prompt-page" id="segment-${activeIndex}">
            <aside class="page02-left-panel">
                <div class="page02-left-title"><strong>场景</strong><span>${activeIndex + 1} / ${segments.length}</span></div>
                <div class="page02-search-box">搜索</div>
                <div class="page02-left-actions"><button type="button" onclick="generateAllVideoPrompts()">批量提示</button><button type="button" onclick="generateAllVideos()">批量视频</button></div>
                <div class="page02-account-tabs"><button class="active">全部场景</button><button>待生成</button><button>已完成</button></div>
                <div class="page02-account-list">${sceneItems}</div>
            </aside>
            <section class="page02-center-panel">
                <div class="page02-editor-toolbar">
                    <h3>视频提示编辑</h3>
                    <div><button type="button" onclick="copyVideoPrompt(${activeIndex})" ${videoPrompt ? '' : 'disabled'}>一键导出</button><button type="button" onclick="saveVideoPromptEdit(${activeIndex})" ${videoPrompt ? '' : 'disabled'}>草稿箱</button><button type="button" onclick="showToast('暂无历史记录', 'info')">历史记录</button></div>
                </div>
                <div class="page02-title-input">${escHtml(title || '在这里输入标题')}</div>
                <div class="page02-editor-body">
                    <div class="page02-source-preview"><div class="page02-source-card"><strong>分镜图</strong><div>${coverPreview}</div></div><div class="page02-source-card video"><strong>视频预览</strong><div>${videoPreview}</div></div></div>
                    <div class="page02-writing-area"><div class="segment-editor mention-editor stage-video-editor video-generation-prompt-editor" id="segEditor-${activeIndex}" data-rows="18" data-placeholder="从这里开始写视频提示"></div><div class="page02-word-count">字数：${videoPrompt ? videoPrompt.length : 0}/100000</div></div>
                </div>
                <div class="page02-editor-footer"><button type="button" onclick="openMentionPickerForButton(${activeIndex})">@ 插入</button><span>人物、场景、提示词库</span></div>
            </section>
            <aside class="page02-right-panel">
                <div class="page02-settings-title">视频设置</div>
                <div class="page02-setting-block"><strong>封面</strong><div class="page02-cover-tabs"><button class="active">使用分镜图</button><button>使用标题图</button><button>不使用</button></div><div class="page02-cover-box">${coverPreview}<small>从素材库选择</small></div></div>
                <div class="page02-setting-block"><strong>摘要</strong><textarea rows="4" placeholder="请输入摘要">${escHtml(title)}</textarea></div>
                <div class="page02-setting-block"><strong>生成参数</strong>
                    <div class="page02-param-row"><label>模式</label><select id="segMode-${activeIndex}" class="seg-mode-select" onchange="onSegModeChange(${activeIndex})"><option value="text_to_video" ${mode==='text_to_video'?'selected':''}>文生视频</option><option value="omni_reference" ${mode==='omni_reference'?'selected':''}>参考图</option></select></div>
                    <div class="page02-param-row"><label>比例</label><select id="segRatio-${activeIndex}" class="seg-small-select">${['16:9','9:16','1:1','4:3','3:4','21:9','adaptive'].map(r => `<option value="${r}" ${ratio === r ? 'selected' : ''}>${r}</option>`).join('')}</select></div>
                    <div class="page02-param-row"><label>时长</label><input type="number" id="segDuration-${activeIndex}" min="4" max="15" value="${duration}" class="seg-duration-input"></div>
                    <div id="segRefWrap-${activeIndex}" class="page02-upload-row ${mode==='text_to_video'?'hidden':''}"><span>参考图</span><input type="file" id="segFile-${activeIndex}" accept="image/*" multiple onchange="onSegmentFilesChange(${activeIndex})"><span id="segFileCount-${activeIndex}" class="segment-file-count">${seg._uploadFileCount || 0}张</span></div>
                </div>
                <div class="page02-publish-actions"><button type="button" class="primary" onclick="generateSingleVideo(${activeIndex})" ${canGenerate ? '' : 'disabled'}>立即生成</button><button type="button" onclick="generateAllVideos()">批量生成</button><button type="button" onclick="saveVideoPromptEdit(${activeIndex})" ${videoPrompt ? '' : 'disabled'}>保存草稿</button></div>
                ${videoProgress}
            </aside>
        </div>`;
}

...
function sanitizeSegmentSourceText(text = '', seg = null) {
    ...
    if (!value) return '';
    const fallback = String(seg?._original || seg?.source_script || '').trim();
    if (isGeneratedVideoPromptText(value, seg)) return fallback;
    if (isLegacyStoryboardPrompt(value) || isTargetStoryboardPromptFormat(value)) return fallback;
    return value;
}
...
async function reprocessSingleSegment(index) {
    ...
    try {
        ...
        currentSegments[index].video_prompt_text = updatedSegment.video_prompt_text || updatedSegment.prompt_text || updatedSegment.prompt || data.result || currentSegments[index].video_prompt_text || '';
...
async function viewProject(projectId, options = {}) {
    try {
        ...
        currentSegments = segments.map(s => ({
            ...
            video_prompt_text: s.video_prompt_text || (isGeneratedVideoPromptText(s.prompt_text || s.prompt || '', s) ? (s.prompt_text || s.prompt || '') : ''),

...
let editStudioActiveClipId = '';
let editStudioDragClipId = '';
let editStudioPreviewTimer = null;
let scriptToNarrativeSkillCache = '';
let scriptBackboneCompactSkillCache = '';
let dialogueStoryboardPromptSkillCache = '';
let seedanceDialogueSkillCache = '';
...
const EDIT_STUDIO_STORAGE_KEY = 'ai-production-edit-studio-state-v1';
const PIPELINE_SKILL_OUTPUT_STORAGE_KEY = 'ai-pipeline-skill-outputs-v1';
const SCRIPT_TO_NARRATIVE_SKILL_URL = '/static/script-to-narrative-skill.md?v=20260625-socket-clean';
const SCRIPT_BACKBONE_COMPACT_SKILL_URL = '/static/script-backbone-compact-skill.md?v=20260625-socket-clean';
const DIALOGUE_STORYBOARD_PROMPT_SKILL_URL = '/static/dialogue-storyboard-prompt-skill.md?v=20260625-storyboard-tags';
const SEEDANCE_DIALOGUE_SKILL_URL = '/static/seedance-dialogue-skill.md?v=20260625-video-skill-socket';
...
function getFullscreenImageSrc(img) {
    if (!img) return '';
    return img.currentSrc || img.src || img.getAttribute('src') || '';
}

function isDirectFullscreenImage(img) {
    if (!img || img.tagName !== 'IMG') return false;
    if (!getFullscreenImageSrc(img)) return false;
    return !!img.closest([
        '.resource-img',
        '.storyboard-image-preview-frame',
        '.storyboard-preview',
        '.stage-image-preview',
        '.edit-studio-preview',
        '.edit-studio-scene-thumb',
        '.storyboard-image-version-thumb',
        '.storyboard-image-nav-thumb',
        '.script-prompt-card-thumb',
        '.video-generation-timeline-thumb',
        '.workbench-segment-thumb',
        '.prompt-library-image',
        '.upload-area',
        '.ue5-json-role-preview',
        '.reference-3d-skybox-slot'
    ].join(','));
}
...
function openImageFullscreen(src, alt = '') {
    ...
    modal.innerHTML = `
        <button type="button" class="image-preview-close" aria-label="关闭预览" onclick="closeImageFullscreen()">×</button>
        <img src="${escAttr(src)}" alt="${escAttr(alt || '图片预览')}">
    `;
...
function closeImageFullscreen() {
    ...
    if (document.fullscreenElement?.id === 'imagePreviewModal') {
        document.exitFullscreen?.().catch(() => {});
    }
}

function initDirectImageFullscreen() {
    document.addEventListener('click', (event) => {
        const img = event.target?.closest?.('img');
        if (!isDirectFullscreenImage(img)) return;
        event.preventDefault();
        event.stopPropagation();
        openImageFullscreen(getFullscreenImageSrc(img), img.alt || '');
    }, true);
    document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape') closeImageFullscreen();
    });
    document.addEventListener('fullscreenchange', () => {
        if (!document.fullscreenElement && document.getElementById('imagePreviewModal')) {
            closeImageFullscreen();
        }
    });
}
...
function getStrictAssetIdsForText(type, text, options = {}) {
    const items = getAssetItems(type);
    const compact = normalizeAssetMatchText(text);
    const candidateIds = new Set(items.map(item => item.id));
    const forcedIds = parseAssetIdList(options.forceIds).filter(id => candidateIds.has(id));
    const matched = items
        .filter(item => {
            const name = String(item.name || '').trim();
            return name && compact.includes(normalizeAssetMatchText(name));
        })
        .map(item => item.id);
    return Array.from(new Set(forcedIds.concat(matched)));
}

function getStoryboardAssetTagScope(text, options = {}) {
    const segment = options.segment || {};
    return {
        scenes: getStrictAssetIdsForText('scene', text, {
            forceIds: segment.scene_id ? [segment.scene_id] : []
        }),
        characters: getStrictAssetIdsForText('char', text, {
            forceIds: parseAssetIdList(segment.character_ids)
        })
    };
}

function buildStoryboardMentionTagLine(assetScope = {}) {
    const charTags = (assetScope.characters || [])
        .map(id => {
            const asset = getAssetById('char', id);
            return asset ? `@char:${id}=${asset.name || id}` : '';
        })
        .filter(Boolean);
    const sceneTags = (assetScope.scenes || [])
        .map(id => {
            const asset = getAssetById('scene', id);
            return asset ? `@scene:${id}=${asset.name || id}` : '';
        })
        .filter(Boolean);
    const tags = charTags.concat(sceneTags);
    return tags.length ? `【参考图Tag】\n${tags.join(' ')}` : '';
}
...
function buildAssetScopeInstruction(scope, label = '当前片段') {
    ...
    return [
        ...
        charNames.length ? `可用相关人物：${charNames.join('、')}。如果当前流程提供 @char:id / @scene:id tag，最终提示词保留 tag；正文直接使用角色名，不要写【图#】。` : '如果当前片段没有明确人物素材，不要编造角色参考图。',
        sceneNames.length ? `可用相关场景：${sceneNames.join('、')}。只保留当前片段实际地点和必要连续性锚点。` : '如果当前片段没有明确场景素材，不要列出整套场景库。',
        '最终 Image Prompt 使用自然中文成品格式；开头直接写“参考图对应：角色名，角色名。”和“生成单张六格动态分镜表...”。',
        '人物库和场景库只作为名称锁定与连续性参考，最终正文直接写人名/场景名，不使用【图#】、char:id、素材编号或网页字段。',
        '禁止输出网页模板标题或旧分块标题：不得出现“固定视觉风格：”“分镜分格要求：”“原始台本依据：”“当前镜头内容：”“逐格镜头要求：”。台词只能转成动作、表情、口型、视线和调度，不能复述台词。'
    ].join('\n');
}

function buildAssetTagLine(assetScope = {}) {
    const charNames = getAssetNamesByIds('char', assetScope.characters);
    const sceneNames = getAssetNamesByIds('scene', assetScope.scenes);
    return [
        charNames.length ? `参考图对应：${charNames.join('，')}` : '',
        sceneNames.length ? `场景名称：${sceneNames.join('，')}` : ''
    ].filter(Boolean).join('。') + (charNames.length || sceneNames.length ? '。' : '');
}

async function loadDialogueStoryboardPromptSkill() {
    if (dialogueStoryboardPromptSkillCache) return dialogueStoryboardPromptSkillCache;
    try {
        const res = await fetch(DIALOGUE_STORYBOARD_PROMPT_SKILL_URL, { cache: 'no-store' });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        dialogueStoryboardPromptSkillCache = (await res.text()).trim();
    } catch (e) {
        console.warn('读取 dialogue-storyboard-prompt skill 失败，使用内置最小插口规则。', e);
        dialogueStoryboardPromptSkillCache = getMainlineStoryboardPromptInstruction();
    }
    return dialogueStoryboardPromptSkillCache;
}

function buildDialogueStoryboardPromptInput({ segmentText = '', backboneText = '', assetScope = {} } = {}) {
    const storyboardTags = buildStoryboardMentionTagLine(assetScope);
    return [
        '【网页工具输入：纯插口模式】',
        storyboardTags,
        backboneText ? backboneText.trim() : '',
        '---',
        `【当前段落】\n${segmentText.trim()}`
    ].filter(Boolean).join('\n\n');
}

async function buildDialogueStoryboardPromptInstruction() {
    const skillText = await loadDialogueStoryboardPromptSkill();
    return [
        skillText,
        '',
        '【本网页插口约束】',
        '上面的 skill 是唯一分镜图提示词规则。不要混入导演模式、旧网页模板、【图#】、素材编号、原始台本依据或后端任务字段。',
        '如果输入里有【参考图Tag】，最终分镜提示词开头必须原样保留这些 @char:id / @scene:id tag；它们用于链接 reference photo、人物信息和地点信息。tag 后面可以写中文名，但不要改 id，不要删 tag。',
        '输入里【剧情主干】只作为后台主干上下文，最终提示词不要出现 script-backbone-compact 字样，也不要把【剧情主干】标题原样输出。',
        '只根据【当前段落】和其对应主干生成一条最终 Image 2 分镜提示词；不要根据对白里“大家/众人/队伍”等普通词自动添加“人群”角色或素材库人物。',
        '格数只由 skill 和当前主干决定：普通连续场景可六格，动作冲突可九格，复杂打斗/多人混战/救援反转优先十二格。'
    ].join('\n');
}

function ensureStoryboardAssetTags(prompt = '', assetScope = {}) {
    const text = String(prompt || '').trim();
    if (!text) return text;
    const charNames = getAssetNamesByIds('char', assetScope.characters);
    const sceneNames = getAssetNamesByIds('scene', assetScope.scenes);
    const needsReference = (assetScope.characters || []).length && !/^参考图对应：/.test(text);
    const needsScene = (assetScope.scenes || []).length && !new RegExp(sceneNames.map(name => name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')).test(text);
    if (!needsReference && !needsScene) return text;
    const tagLine = [
        needsReference && charNames.length ? `参考图对应：${charNames.join('，')}` : '',
        needsScene && sceneNames.length ? `场景名称：${sceneNames.join('，')}` : ''
    ].filter(Boolean).join('。') + '。';
    if (tagLine === '。') return text;
    if (/^参考图对应：[^。]+。/.test(text)) {
        return text.replace(/^(参考图对应：[^。]+。)/, `$1${tagLine}`);
    }
    return `${tagLine}${text}`;
}
...
function buildMainlineSegmentationRule() {
    return [
        '只用于第一页台本分段：必须按 film-shot-timing-reference 规则综合估算台词、动作、可读性和情绪修正，再切成15秒以内原台词段落。',
        ...
        '只计算引号内实际发声的中文字符、英文单词或类似音节的发声单位、数字和会读出的符号；忽略标点、空格和非发声符号。',
        '每行综合计时 = max(台词朗读时间, 动作执行时间, 2秒可读性底线) × 情绪修正；动作和台词重叠时取 max，不要累加。',
        '动作查表：转头1.5-2秒，低头/抬头1-2秒，坐下/站起2-3秒，走过房间4-6秒，转身离开3-5秒，拔枪/刀约1秒，拥抱2-4秒，开门进入2.5-4秒，摔倒1.5-2.5秒，挥拳/砍劈0.5-1.2秒，刺/捅0.3-0.8秒。',
        '情绪修正：愤怒/急促×0.6-0.8，恐惧/紧张×0.7-0.9，悲伤/压抑×1.2-1.5，迟疑×1.1-1.3，震惊/崩溃×1.3-1.8。',
        '每个视频片段目标接近但不超过15秒。短台词可以按顺序合并到同一个片段，直到下一句会超过15秒才切开；动作/沉默/环境行也要计时，不能免费塞进上一段。单句长台词超过15秒时，才按自然停顿拆开。',
        '输出只保留分段后的原台词/舞台指示，不生成 Image Prompt、分镜图提示词或 Video Prompt。'
    ].join('\n');
}

async function loadPipelineSkillText(url, cacheName, fallbackText = '') {
    if (cacheName === 'scriptToNarrative' && scriptToNarrativeSkillCache) return scriptToNarrativeSkillCache;
    if (cacheName === 'scriptBackboneCompact' && scriptBackboneCompactSkillCache) return scriptBackboneCompactSkillCache;
    try {
        const res = await fetch(url, { cache: 'no-store' });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const text = (await res.text()).trim();
        if (cacheName === 'scriptToNarrative') scriptToNarrativeSkillCache = text;
        if (cacheName === 'scriptBackboneCompact') scriptBackboneCompactSkillCache = text;
        return text;
    } catch (e) {
        console.warn(`读取 ${cacheName} skill 失败，使用最小兜底规则。`, e);
        return fallbackText;
    }
}

async function buildScriptToNarrativeInstruction() {
    const skillText = await loadPipelineSkillText(
        SCRIPT_TO_NARRATIVE_SKILL_URL,
        'scriptToNarrative',
        '执行 script-to-narrative skill：把完整台本表格/台本输入转成连续 detailed writing 剧本。只输出剧本，不输出分析、解释或下一步指引。'
    );
    return [
        skillText,
        '',
        '【网页插口约束】',
        '上面的 script-to-narrative skill 是唯一规则；不要混入分段规则、分镜图规则、视频提示词规则、人物库、场景库、导演模式或网页旧模板。',
        '输入只来自用户原台本。输出只是一段剧本 detailed writing；不得输出 ===SCRIPT_SPLIT===、分段主干、Image Prompt、Video Prompt、Markdown 标题或表格。',
        '如果原台本是表格，按 Text Window + Context Description + Character 融合；如果是普通文本，按原文顺序转成连续动作叙事。'
    ].join('\n');
}

async function buildScriptBackboneCompactInstruction(index, segmentText, narrativeText) {
    const skillText = await loadPipelineSkillText(
        SCRIPT_BACKBONE_COMPACT_SKILL_URL,
        'scriptBackboneCompact',
        '执行 script-backbone-compact skill：基于分段原文 + 剧本详细版，输出每段迷你主干。'
    );
    return [
        skillText,
        '',
        '【网页插口约束】',
        '上面的 script-backbone-compact skill 是唯一规则；不要混入分镜图 prompt、视频 prompt、人物库、场景库、导演模式或网页旧模板。',
        '只处理当前一个分段；输出必须是一个独立主干块，直接以 ---分段N--- 开头；不要输出其他分段。',
        '字段名必须使用 skill 的格式：【剧情主干】【角色】【场景】【时间】【道具】【情绪】。不要改成【固定道具】或【情绪弧线】。',
        '全部使用短语/关键词，不写完整解释；不要把原台词整段复制进主干。',
        '---',
        '当前段落：',
        `分段原文：\n${segmentText}`,
        '',
        `剧本详细版：\n${narrativeText || segmentText}`
    ].join('\n');
}

async function requestPipelineText(script, instruction, progressKey = '', progressLabel = '模型处理中') {
    ...
    await readNdjsonStream(res, {
        status(event) {
            if (progressKey) updateInlineLiveProgress(progressKey, { stage: event.stage, text: event.message || progressLabel });
        },
        delta(event) {
            streamedText += event.text || '';
            if (progressKey) {
                updateInlineLiveProgress(progressKey, {
                    stage: 'streaming',
                    text: `${progressLabel}，已收到 ${event.char_count || streamedText.length} 字`,
                    charCount: event.char_count || streamedText.length
                });
            }
        },
        done(event) {
    ...
    return String(finalData?.result || streamedText || '').trim();
}

async function generateScriptNarrative(script, extraInstruction = '', progressKey = '') {
    return requestPipelineText(
        script,
        await buildScriptToNarrativeInstruction(),
        progressKey,
        'script-to-narrative 正在生成剧本详细版'
    );
}

function getPipelineSkillOutputStorageKey() {
    return `${PIPELINE_SKILL_OUTPUT_STORAGE_KEY}:${currentProject || 'draft'}`;
}

function getSegmentBackboneKey(seg, index) {
    return String(seg?.id || seg?.segment_id || seg?.segment_index || index);
}

function loadPipelineSkillOutputs() {
    try {
        const raw = localStorage.getItem(getPipelineSkillOutputStorageKey());
        const data = raw ? JSON.parse(raw) : {};
        return {
            narrative: String(data.narrative || currentScriptNarrative || ''),
            backbones: data.backbones && typeof data.backbones === 'object' ? data.backbones : {}
        };
    } catch (e) {
        return { narrative: currentScriptNarrative || '', backbones: {} };
    }
}

function savePipelineSkillOutputs(patch = {}) {
    const previous = loadPipelineSkillOutputs();
    const next = {
        narrative: patch.narrative !== undefined ? String(patch.narrative || '') : previous.narrative,
        backbones: { ...previous.backbones, ...(patch.backbones || {}) }
    };
    try {
        localStorage.setItem(getPipelineSkillOutputStorageKey(), JSON.stringify(next));
    } catch (e) {
        console.warn('保存 skill 输出快照失败', e);
    }
    if (next.narrative) currentScriptNarrative = next.narrative;
    return next;
}

function resetPipelineSkillOutputs(narrative = '') {
    const next = { narrative: String(narrative || ''), backbones: {} };
    try {
        localStorage.setItem(getPipelineSkillOutputStorageKey(), JSON.stringify(next));
    } catch (e) {
        console.warn('重置 skill 输出快照失败', e);
    }
    currentScriptNarrative = next.narrative;
    return next;
}

function hydratePipelineSkillOutputs(segments = currentSegments) {
    const outputs = loadPipelineSkillOutputs();
    if (outputs.narrative && !currentScriptNarrative) currentScriptNarrative = outputs.narrative;
    segments.forEach((seg, index) => {
        if (!seg) return;
        const key = getSegmentBackboneKey(seg, index);
        const source = getSegmentSourceText(seg);
        const savedBackbone = outputs.backbones[key];
        if (!seg._fullNarrative && (currentScriptNarrative || outputs.narrative)) seg._fullNarrative = currentScriptNarrative || outputs.narrative;
        if (!seg._backboneCompact && savedBackbone && typeof savedBackbone === 'object' && savedBackbone.source === source) {
            seg._backboneCompact = savedBackbone.text || '';
            seg._backboneSourceText = savedBackbone.source || '';
        }
    });
}

async function generateSegmentBackbone(index, segmentText, narrativeText, options = {}) {
    const seg = currentSegments[index];
    const source = String(segmentText || '').trim();
    if (seg?._backboneCompact && seg?._backboneSourceText === source) return seg._backboneCompact;
    if (!source) return '';
    try {
        const backbone = await requestPipelineText(
            [
                `分段原文：\n${source}`,
                '',
                `剧本详细版：\n${narrativeText || currentScriptNarrative || source}`
            ].join('\n'),
            await buildScriptBackboneCompactInstruction(index, source, narrativeText || currentScriptNarrative || source),
            '',
            `script-backbone-compact 正在提炼分段 ${index + 1}`
        );
        if (seg) {
            seg._backboneCompact = backbone;
            seg._backboneSourceText = source;
        }
        if (seg && backbone) {
            savePipelineSkillOutputs({ backbones: { [getSegmentBackboneKey(seg, index)]: { source, text: backbone } } });
            renderPipelineSkillOutputSlot();
        }
        return backbone;
    } catch (e) {
        console.warn(`分段 ${index + 1} 主干提炼失败，继续生成 Image Prompt:`, e);
        if (!options.silentLoading) showToast(`分段 ${index + 1} 主干提炼失败，已跳过`, 'warning');
        return '';
    }
}
...
const ACTION_TIMING_TABLE = [
    { re: /转头|回头|扭头/, min: 1.5, max: 2.0 },
    { re: /低头|抬头/, min: 1.0, max: 2.0 },
    { re: /坐下|坐.*下/, min: 2.0, max: 3.0 },
    { re: /站起|起身|站起来/, min: 2.0, max: 3.0 },
    { re: /走过.*房间|走过.*厅|穿过.*房间|穿过.*大厅/, min: 4.0, max: 6.0 },
    { re: /转身离开|转身.*走|转身.*离去/, min: 3.0, max: 5.0 },
    { re: /拔.*(枪|刀|剑|武器)|掏.*(枪|刀|剑|武器)/, min: 0.8, max: 1.0 },
    { re: /扇.*(巴掌|耳光)/, min: 0.8, max: 1.5 },
    { re: /拥抱/, min: 2.0, max: 4.0 },
    { re: /握手/, min: 1.5, max: 2.5 },
    { re: /开.*门.*进|推门.*入|开门.*跨|推门.*进/, min: 2.5, max: 4.0 },
    { re: /走.*几步|前进.*几步|跨出.*步/, min: 1.0, max: 2.0 },
    { re: /冲.*出|冲出|狂奔|飞奔/, min: 1.0, max: 2.5 },
    { re: /跳|跃|蹦/, min: 0.5, max: 1.5 },
    { re: /摔倒|倒下|跌倒|跪倒/, min: 1.5, max: 2.5 },
    { re: /后退.*步|退.*步|后撤.*步/, min: 1.0, max: 2.0 },
    { re: /挥.*(拳|刀|剑|手臂)|砍|劈/, min: 0.5, max: 1.2 },
    { re: /刺|捅/, min: 0.3, max: 0.8 },
    { re: /举起|抬起.*手|伸手/, min: 0.8, max: 1.5 },
    { re: /放下.*手|手臂.*落下/, min: 0.6, max: 1.2 }
];

const EMOTION_MODIFIER_TABLE = [
    { re: /愤怒|暴怒|怒火/, coef: 0.7 },
    { re: /急促|匆忙|慌张|喊|大喊|冲口|抢话|吼|赶紧|立刻|马上/, coef: 0.75 },
    { re: /恐惧|惊恐|害怕|紧张/, coef: 0.85 },
    { re: /悲伤|伤心|难过|哽咽|落泪/, coef: 1.35 },
    { re: /压抑|沉重|阴郁/, coef: 1.3 },
    { re: /迟疑|犹豫/, coef: 1.2 },
    { re: /震惊|惊呆|惊愕/, coef: 1.5 },
    { re: /崩溃|绝望|痛哭/, coef: 1.6 },
    { re: /沉默|停顿|静默/, coef: 1.25 },
    { re: /疲惫|疲倦|累了/, coef: 1.2 }
];

function estimateActionSeconds(text = '') {
    const raw = String(text || '');
    let best = 0;
    ACTION_TIMING_TABLE.forEach(row => {
        if (row.re.test(raw)) best = Math.max(best, (row.min + row.max) / 2);
    });
    if (!best && /舞台指示|情景描述|场景|动作|镜头|环境|音效|音乐|看向|望向|拿起|放下|递给|靠近|远离|环顾|凝视|对视|皱眉|微笑|叹气|点头|摇头/.test(raw)) {
        best = 2.0;
    }
    if (/新场景|新角色|第一次出现|建立|全景|远景|环境/.test(raw)) best += 1.5;
    if (/线索|道具|关键|重要|发现|揭露|反转/.test(raw)) best += 1.0;
    return best;
}

function getEmotionModifier(text = '') {
    const raw = String(text || '');
    const row = EMOTION_MODIFIER_TABLE.find(item => item.re.test(raw));
    return row ? row.coef : 1.0;
}

function estimateLineDurationSeconds(text = '') {
    const raw = String(text || '').trim();
    if (!raw) return 0;
    let speech = estimateSpeechSeconds(raw);
    if (!speech && isDialogueContinuationLine(raw)) {
        speech = estimateSpeechSeconds(`继续说道：“${raw}”`);
    }
    const action = estimateActionSeconds(raw);
    const modifier = getEmotionModifier(raw);
    return Math.max(2.0, Math.max(speech, action) * modifier);
}
...
function expandLongDialogueLineForTiming(line = '') {
    const raw = String(line || '').trim();
    const baseSeconds = estimateLineDurationSeconds(raw);
...
function estimateSegmentDurationSeconds(text = '') {
    ...
    let totalSeconds = 0;
    lines.forEach(line => {
        totalSeconds += estimateLineDurationSeconds(line);
    });
    return Math.min(15, Math.max(4, Math.ceil(totalSeconds)));
}
...
function splitScriptByTimingRule(script = '') {
    ...
    lines.forEach(line => {
        const lineSeconds = estimateLineDurationSeconds(line);
        const projectedDuration = currentSeconds + lineSeconds;
        if (current.length && projectedDuration > 15) {
            flush();
        }
...
async function processScript() {
    ...
    const assetScope = storyboardMainline ? { scenes: [], characters: [] } : getScopedAssetPayload(script);
    ...
    if (!storyboardMainline && !directorModeId) { showToast('请选择视频模式', 'warning'); resetBtn(); return; }
    ...
    try {
        ...
        currentScriptNarrative = '';
        if (storyboardMainline) {
            updateInlineLiveProgress('process-script', { stage: 'script-to-narrative', text: 'script-to-narrative：正在把台本转成剧本详细版' });
            narrativeScript = await generateScriptNarrative(script, extraInstruction, 'process-script');
            currentScriptNarrative = narrativeScript || script;
            renderPipelineSkillOutputSlot();
        }
        const timingSourceScript = script;
        const timedSplitParts = storyboardMainline ? splitScriptByTimingRuleParts(timingSourceScript) : [];
        const timedSplitScript = storyboardMainline ? timedSplitParts.join('\n\n===SCRIPT_SPLIT===\n\n') : script;
        const mainlineInstruction = storyboardMainline
            ? `${buildMainlineImagePromptInstruction(extraInstruction)}\n\n前端已先执行 script-to-narrative，把台本转为剧本详细版，作为后续 script-backbone-compact 和分镜提示词的上下文参考；但 01 分段必须直接按原台词/原台本执行 film-shot-timing-reference，并用 ===SCRIPT_SPLIT=== 预切分。必须严格保留这些原台词片段边界，不要合并，不要重新分段。输出每段时只保留该段原台词/原台本文字，不要输出 script-to-narrative 改写后的内容，不生成 Image Prompt。`
            : extraInstruction;
        ...
        const res = await fetch('/api/process-script/stream', {
            ...
            body: JSON.stringify({
                ...
                narrative_script: currentScriptNarrative,
                presegmented_script: timedSplitScript,
                scenes: storyboardMainline ? [] : assetScope.scenes,
                characters: storyboardMainline ? [] : assetScope.characters,
                director_mode_id: storyboardMainline ? '' : directorModeId,
        ...
        if (storyboardMainline) resetPipelineSkillOutputs(currentScriptNarrative);
        ...
        if (storyboardMainline) {
            ...
            currentSegments = timedSplitParts.map((part, index) => {
                ...
                return {
                    ...
                    _fullNarrative: currentScriptNarrative,
                    _backboneCompact: '',
                    storyboard_prompt: '',
                    storyboard_status: '',
...
function cacheActiveSegmentEditorState(index = activeSegmentEditIndex) {
    ...
    if (videoPromptEl) {
        ...
        if (videoPrompt.trim()) {
            seg.video_prompt_text = videoPrompt;
            seg._videoPromptDirty = true;
        }
    }
}
...
function getSegmentStoryTitle(seg, index) {
    const text = getSegmentSourceText(seg).replace(/\r/g, '').trim();
...
function getSegmentSourceText(seg) {
    return sanitizeSegmentSourceText(seg?._original || seg?.source_script || seg?.prompt_text || seg?.prompt || '', seg);
}

function isGeneratedBackboneText(text = '') {
    const value = String(text || '').trim();
    if (!value) return false;
    return /script-backbone-compact|分段主干|剧情主干|【剧情主干】|【角色】|【道具】|【情绪】|【固定道具】|【情绪弧线】|【分段】|入场方式|主干提炼|人物-动作链|人物动作链|场景锚点|连续性锚点|镜头主干|核心事件|情绪转折|^\s*-{2,}\s*分段\s*\d+/i.test(value);
}

function isGeneratedStoryboardPromptText(text = '') {
    const value = String(text || '').trim();
    if (!value) return false;
    return isLegacyStoryboardPrompt(value)
        || isTargetStoryboardPromptFormat(value)
        || /参考图对应|人物Tag|场景Tag|生成单张.{0,20}分镜表|生成一张\s*\d+\s*格|黑白铅笔分镜|角色锁|背景锁|负面提示|镜头路径箭头|三乘二清晰分格|P0?1[：:，, ]|01[：:，,].{0,80}02[：:，,]/.test(value);
}

function isGeneratedVideoPromptRaw(text = '') {
    const value = String(text || '').trim();
    if (!value) return false;
    return /Video Prompt|Seedance|视频提示词|视频生成|目标时长|连续视频|Dynamic Description|Static Description|动态描述|静态描述|风格与氛围|音频|运镜|口型|无字幕|分镜图参考|构图锚点/.test(value);
}

function getStableSegmentOriginal(seg = null) {
    const candidates = [seg?._original, seg?.source_script, seg?.prompt_text, seg?.prompt];
    for (const item of candidates) {
        const value = String(item || '').trim();
        if (!value) continue;
        if (isGeneratedBackboneText(value) || isGeneratedStoryboardPromptText(value) || isGeneratedVideoPromptRaw(value)) continue;
        return value;
    }
    return '';
}

function sanitizeSegmentSourceText(text = '', seg = null) {
    const value = String(text || '').trim();
    if (!value) return '';
    const fallback = getStableSegmentOriginal(seg);
    if (isGeneratedBackboneText(value)) return fallback;
    if (isGeneratedVideoPromptText(value, seg) || isGeneratedVideoPromptRaw(value)) return fallback;
    if (isGeneratedStoryboardPromptText(value)) return fallback;
    return value;
}
...
function getProtectedSegmentSourcePatch(currentSeg = {}, fallbackSeg = {}) {
    const source = getStableSegmentOriginal(currentSeg) || getStableSegmentOriginal(fallbackSeg) || '';
    if (!source) return {};
    return {
        prompt: source,
        prompt_text: source,
...
function isTargetStoryboardPromptFormat(prompt = '') {
    const value = String(prompt || '').trim();
    const tagPrefix = String.raw`(?:@(?:char|scene):[A-Za-z0-9_\-]{1,64}(?:=[^\s。]+)?[\s。；;，,、]*)*`;
    const referencePrefix = String.raw`(?:参考图对应：[^。]+。)?`;
    const prefix = new RegExp(`^${tagPrefix}${referencePrefix}`);
    const body = value.replace(prefix, '');
    return (/^生成单张(四格|六格|九格|十二格)动态分镜表/.test(body)
        || /^(黑白铅笔分镜|单色石墨草图|灰阶明暗|生成一张\s*(4|6|9|12|四|六|九|十二)\s*格)/.test(body))
        && /(负面提示|不要静态合照|无字幕)/.test(value)
        && !isLegacyStoryboardPrompt(value);
}

function getStoryboardPromptFormatRetryInstruction() {
    return [
        '上一版输出仍然像旧网页模板，必须按“对话分镜提示词”skill 重写。',
        '开头必须直接进入最终分镜提示词；格数按 skill 和当前主干判断，不要固定套六格。',
        '如果当前插口输入里有 @char:id / @scene:id，最终提示词开头必须保留这些 tag；不要写【图#】或素材编号。',
        '固定风格必须并入开头长句，禁止出现“固定视觉风格：”。',
        '必须写“根据以下剧情转化镜头，不复述台词：...”并用视觉剧情概括，不要粘贴原台词。',
        '逐格部分按实际格数输出 01、02、03……；复杂打斗/多人混战/救援反转优先十二格。',
        '禁止出现：原始台本依据、当前镜头内容、逐格镜头要求、分镜分格要求、固定视觉风格。'
    ].join('\n');
}

function isLikelySourceScriptText(text = '', seg = null) {
    const value = String(text || '').trim();
    if (!value) return false;
    const source = (seg?._original || seg?.source_script || '').trim();
    if (source && value === source) return true;
    return /===SCRIPT_SPLIT===|Text Window|Context Description|Character|Dialogue|原台本|原片台词|段落标题|场景\/时空|出场人物|剧情目标|动作与对白|关键道具|首帧锚定依据|尾帧衔接依据/i.test(value);
}

function isGeneratedVideoPromptText(text = '', seg = null) {
    const value = String(text || '').trim();
    if (!value || isLikelySourceScriptText(value, seg)) return false;
    return /Video Prompt|Seedance|视频提示词|视频生成|目标时长|连续视频|Dynamic Description|Static Description|动态描述|静态描述|风格与氛围|音频|运镜|口型|无字幕|分镜图参考|构图锚点/i.test(value);
}

function getSegmentVideoPrompt(seg) {
    if (!seg) return '';
    const explicit = (seg.video_prompt_text || seg.video_prompt || '').trim();
    if (explicit && !isLikelySourceScriptText(explicit, seg)) return explicit;
    const value = (seg.prompt || seg.prompt_text || '').trim();
    if ((seg._videoPromptDirty && !isLikelySourceScriptText(value, seg)) || isGeneratedVideoPromptText(value, seg)) return value;
    return '';
}
...
function getStoryboardImageStatus(seg) {
    if (seg?.storyboard_status === 'generating' && seg?.storyboard_prompt) return renderPipelineStatusPill('running', '正在出图');
...
function getVideoPromptStatus(seg) {
    ...
    if (seg?._videoPromptStatus === 'stale') return renderPipelineStatusPill('idle', '需重生');
...
function getStoryboardVersionStorageKey(seg = {}, index = 0) {
    const projectKey = currentProject || seg.project_id || 'local';
    const segmentKey = seg.id || `index-${index}`;
    return `flow127:storyboard-image-versions:${projectKey}:${segmentKey}`;
}

function getStoryboardActiveVersionStorageKey(seg = {}, index = 0) {
    return `${getStoryboardVersionStorageKey(seg, index)}:active`;
}
...
function normalizeStoryboardImageVersions(seg = {}, index = 0) {
    let stored = [];
    try {
        stored = JSON.parse(localStorage.getItem(getStoryboardVersionStorageKey(seg, index)) || '[]');
    } catch (e) {
        stored = [];
    }
    const source = Array.isArray(seg._storyboardImageVersions) && seg._storyboardImageVersions.length ? seg._storyboardImageVersions : stored;
    const seen = new Set();
    const versions = [];
    source.forEach((item, itemIndex) => {
        const path = item?.path || item?.storyboard_image_path || item?.url || '';
        if (!path || seen.has(path)) return;
        seen.add(path);
        versions.push({
            id: item.id || `v-${itemIndex + 1}-${Math.abs(hashString(path))}`,
            path,
            label: item.label || `版本 ${versions.length + 1}`,
            created_at: item.created_at || item.createdAt || ''
        });
    });
    if (seg.storyboard_image_path && !seen.has(seg.storyboard_image_path)) {
        versions.push({
            id: `v-${versions.length + 1}-${Date.now()}`,
            path: seg.storyboard_image_path,
            label: `版本 ${versions.length + 1}`,
            created_at: new Date().toISOString()
        });
    }
    seg._storyboardImageVersions = versions.map((item, itemIndex) => ({
        ...item,
        label: `版本 ${itemIndex + 1}`
    }));
    let storedActivePath = '';
    try {
        storedActivePath = localStorage.getItem(getStoryboardActiveVersionStorageKey(seg, index)) || '';
    } catch (e) {
        storedActivePath = '';
    }
    if (!seg.storyboard_image_path && storedActivePath && seen.has(storedActivePath)) {
        seg.storyboard_image_path = storedActivePath;
    }
    seg._activeStoryboardImageVersionPath = seg.storyboard_image_path || storedActivePath || seg._activeStoryboardImageVersionPath || '';
    try {
        localStorage.setItem(getStoryboardVersionStorageKey(seg, index), JSON.stringify(seg._storyboardImageVersions));
        localStorage.setItem(getStoryboardActiveVersionStorageKey(seg, index), imagePath);
    } catch (e) {
        console.warn('保存分镜图版本失败', e);
    }
    return seg._storyboardImageVersions;
}

function rememberStoryboardImageVersion(index, imagePath, meta = {}) {
    const seg = currentSegments[index];
    if (!seg || !imagePath) return [];
    const versions = normalizeStoryboardImageVersions(seg, index);
    if (!versions.some(item => item.path === imagePath)) {
        versions.push({
            id: `v-${versions.length + 1}-${Date.now()}`,
            path: imagePath,
            label: `版本 ${versions.length + 1}`,
    ...
    seg._activeStoryboardImageVersionPath = imagePath;
    try {
        localStorage.setItem(getStoryboardVersionStorageKey(seg, index), JSON.stringify(seg._storyboardImageVersions));
    } catch (e) {
        console.warn('保存分镜图版本失败', e);
...
function renderStoryboardImageVersions(index, seg = {}) {
    const versions = normalizeStoryboardImageVersions(seg, index);
    if (!versions.length) return '';
    const activePath = seg.storyboard_image_path || '';
    return `
        <div class="storyboard-image-version-history">
            <div class="storyboard-image-version-head">
                <strong>分镜图版本</strong>
                <span>${versions.length} 张 · 点击缩略图切换</span>
            </div>
            <div class="storyboard-image-version-row">
                ${versions.map(item => `
                    <button type="button" class="storyboard-image-version-card ${item.path === activePath ? 'active' : ''}" onclick="selectStoryboardImageVersion(${index}, ${JSON.stringify(item.id).replace(/"/g, '&quot;')})" title="切换到${escAttr(item.label)}">
                        <span class="storyboard-image-version-thumb"><img src="/${escAttr(item.path)}" alt="${escAttr(item.label)}"></span>
...
function applyStoryboardImageVersionToSegment(index, version) {
    const seg = currentSegments[index];
    if (!seg || !version?.path) return false;
    seg.storyboard_image_path = version.path;
    seg.storyboard_status = 'image_ready';
    seg._activeStoryboardImageVersionPath = version.path;
    seg._selectedStoryboardImageVersionId = version.id || '';
    seg._videoPromptStatus = getSegmentVideoPrompt(seg) ? 'stale' : (seg._videoPromptStatus || '');
    seg._videoPromptDirty = false;
    seg.continuity_notes = `已切换到${version.label || '所选版本'}，02 会使用这张分镜图作为视频提示参考。`;
    const imageEls = [
        ...document.querySelectorAll(`#storyboard-image-current-${index}`),
        ...document.querySelectorAll(`#video-prompt-storyboard-image-${index}`),
        ...document.querySelectorAll(`[data-storyboard-current-image="${index}"]`)
    ];
    imageEls.forEach(img => {
        if (img?.tagName === 'IMG') img.src = `/${version.path}`;
    });
    return true;
}

async function selectStoryboardImageVersion(index, versionId) {
    const seg = currentSegments[index];
    if (!seg) return;
    const versions = normalizeStoryboardImageVersions(seg, index);
    const version = versions.find(item => item.id === versionId);
    if (!version) return;
    if (!applyStoryboardImageVersionToSegment(index, version)) return;
    await saveSegmentState(index, {
        storyboard_image_path: version.path,
        storyboard_status: 'image_ready',
        video_prompt_text: seg.video_prompt_text || '',
        continuity_notes: seg.continuity_notes
    }, { skipStoryboardPrompt: true });
    try {
        localStorage.setItem(getStoryboardActiveVersionStorageKey(seg, index), version.path);
    } catch (e) {
        console.warn('保存当前分镜图版本失败', e);
    }
    renderSegments(currentSegments);
    renderStoryboardOnlyList(currentSegments);
    showToast(`已切换到${version.label}，01/02 已同步`, 'success');
}

const STORYBOARD_LIGHTING_PRESETS = [
    { id: 'brightness-fix', label: '整体变亮', prompt: '不改变图片内容、人物、构图、镜头、动作、服装和道具，只调整光影、色彩和曝光。不改变图片内容，整体变亮，一定要用UE5色彩风格，不许用纯黑或死黑阴影，增加曝光补偿。保持分镜连续性和原始构图，不新增人物或物体。' },
    { id: 'sunset-warm-backlight', label: '夕阳暖光', prompt: '不改变图片内容、人物、构图、镜头、动作、服装和道具，只调整光影、色彩和曝光。不改变图片内容，电影感，被金黄时刻光逆光照射，柔和漫射的光线在发丝边缘营造发光的角色光环。保持分镜连续性和原始构图，不新增人物或物体。' },
    { id: 'candle-warm-light', label: '烛火暖光', prompt: '不改变图片内容、人物、构图、镜头、动作、服装和道具，只调整光影、色彩和曝光。不改变图片内容，光照到脸部，室内烛光照明，温橙色，氛围古典，光影跳动，皮肤高光细腻。保持分镜连续性和原始构图，不新增人物或物体。' },
    { id: 'rim-backlight', label: '逆光轮廓光', prompt: '不改变图片内容、人物、构图、镜头、动作、服装和道具，只调整光影、色彩和曝光。不改变图片内容，逆光照明，层次分明，电影感，梦幻，边缘轮廓清晰勾勒。保持分镜连续性和原始构图，不新增人物或物体。' },
    { id: 'forest-dappled-light', label: '林间斑驳光', prompt: '不改变图片内容、人物、构图、镜头、动作、服装和道具，只调整光影、色彩和曝光。不改变图片内容，树叶透光，斑驳光影，自然光，明暗交错，氛围灵动。保持分镜连续性和原始构图，不新增人物或物体。' },
    { id: 'window-side-light', label: '窗户光', prompt: '不改变图片内容、人物、构图、镜头、动作、服装和道具，只调整光影、色彩和曝光。不改变图片内容，窗户侧光，自然光效，画面明亮，柔和层次清晰。保持分镜连续性和原始构图，不新增人物或物体。' },
    { id: 'dreamy-highlight-light', label: '梦幻光', prompt: '不改变图片内容、人物、构图、镜头、动作、服装和道具，只调整光影、色彩和曝光。不改变图片内容，高光闪耀，电影感，梦幻，边缘轮廓清晰勾勒。保持分镜连续性和原始构图，不新增人物或物体。' },
    { id: 'rainy-night-atmosphere-light', label: '雨夜氛围光', prompt: '不改变图片内容、人物、构图、镜头、动作、服装和道具，只调整光影、色彩和曝光。不改变图片内容，整体变亮，雨夜，灯光照明，湿润反射，冷暖对比。保持分镜连续性和原始构图，不新增人物或物体。' }
];

function buildStoryboardLightingEditPrompt(preset) {
    const presetPrompt = String(preset?.prompt || '').trim();
    return [
        '把当前已生成的分镜图作为 reference photo / 原图参考来编辑。',
        '必须严格保留 reference photo 里的分镜表布局、每一格构图、人物身份、服装、道具、动作、镜头角度、景别、前中后景关系和阅读顺序。',
        '不要重新生成新分镜，不要改变任何剧情内容，不要新增人物或物体，不要改变格数。',
        presetPrompt,
        '只做光影、色彩和曝光编辑；保持分镜连续性和原始构图。'
    ].filter(Boolean).join('\n');
}

function renderStoryboardPanelTargetButtons(index, seg = {}, options = {}) {
    const count = Math.max(1, inferStoryboardGridCount(seg));
    const disabled = options.disabled ? 'disabled' : '';
    const buttons = Array.from({ length: count }, (_, n) => {
        const no = n + 1;
        const active = seg._storyboardEditTargetGrid === no ? 'active' : '';
        return `<button type="button" class="storyboard-grid-edit-btn ${active}" data-storyboard-grid-btn="${index}" data-grid-no="${no}" onclick="selectStoryboardEditGrid(${index}, ${no})" ${disabled}>${no}格</button>`;
    }).join('');
    return `
        <div class="storyboard-panel-targets">
            <div class="storyboard-panel-targets-head"><strong>改哪一格</strong><span>选择后只改这一格</span></div>
            <div class="storyboard-grid-edit-row">${buttons}</div>
        </div>`;
}

function renderStoryboardLightingPresets(index, seg = {}, options = {}) {
    const disabled = options.disabled ? 'disabled' : '';
    const buttons = STORYBOARD_LIGHTING_PRESETS.map(preset => {
        const active = seg._storyboardLightingPreset === preset.id ? 'active' : '';
        return `<button type="button" class="storyboard-lighting-btn ${active}" onclick="applyStoryboardLightingPreset(${index}, '${preset.id}')" ${disabled}>${preset.label}</button>`;
    }).join('');
    return `
        <div class="storyboard-lighting-presets">
            <div class="storyboard-lighting-presets-head"><strong>光影</strong><span>只改光影，不改内容</span></div>
            <div class="storyboard-lighting-presets-row">${buttons}</div>
        </div>`;
}

function renderPipelineSkillOutputPanels(segments, activeIndex) {
    hydratePipelineSkillOutputs(segments);
    const narrative = currentScriptNarrative || loadPipelineSkillOutputs().narrative || '';
    const narrativeText = narrative.trim() || '还没有生成。点击“生成分段 Image Prompt”后，这里会显示 script-to-narrative 输出。';
    return `
        <section class="pipeline-skill-output-panel compact">
            <div class="pipeline-skill-output-card narrative">
                <div class="pipeline-skill-card-head">
                    <strong>script-to-narrative 输出</strong>
                    <span>${narrative.trim() ? '已使用' : '待生成'}</span>
                </div>
                <pre>${escHtml(narrativeText)}</pre>
            </div>
        </section>`;
}

function renderPipelineSkillOutputSlot() {
    const slot = document.getElementById('scriptPipelineSkillOutputs');
    if (!slot) return;
    if (!currentSegments.length && !currentScriptNarrative && !loadPipelineSkillOutputs().narrative) {
        slot.innerHTML = '';
        slot.classList.add('hidden');
        return;
    }
    slot.classList.remove('hidden');
    slot.innerHTML = renderPipelineSkillOutputPanels(currentSegments, clampActiveSegmentEditIndex(currentSegments));
}

async function applyStoryboardLightingPreset(index, presetId) {
    const seg = currentSegments[index];
    const preset = STORYBOARD_LIGHTING_PRESETS.find(item => item.id === presetId);
    if (!seg || !preset) return;
    if (!seg.storyboard_image_path) {
        showToast('请先生成分镜图，再使用光影按钮。', 'warning');
        return;
    }
    seg._storyboardLightingPreset = preset.id;
    seg._storyboardEditPrompt = buildStoryboardLightingEditPrompt(preset);
    const editor = document.getElementById(`storyboardEditPrompt-${index}`);
    if (editor) editor.value = seg._storyboardEditPrompt;
    renderSegments(currentSegments);
    renderStoryboardOnlyList(currentSegments);
    await editStoryboardImage(index);
}
...
function renderScriptPromptOverview(segments) {
    ...
    const activePromptLoading = activeSeg.storyboard_status === 'generating' && !activePrompt;
    const activeImageLoading = activeSeg.storyboard_status === 'generating' && !!activePrompt;
    ...
    const activeImagePreview = activeImagePath
        ? `<img id="storyboard-image-current-${activeIndex}" data-storyboard-current-image="${activeIndex}" src="/${escAttr(activeImagePath)}" alt="片段 ${activeIndex + 1} 分镜图">`
        : `<div class="storyboard-image-empty">
                <strong>${activePrompt ? '待出图' : '待生成'}</strong>
                <span>${activePrompt ? '生成后显示分镜图。' : '先生成分镜提示。'}</span>
           </div>`;
    const activeStoryboardGridButtons = activeImagePath ? Array.from({ length: inferStoryboardGridCount(activeSeg) }, (_, n) => {
        const no = n + 1;
        return `<button type="button" class="storyboard-grid-edit-btn ${activeSeg._storyboardEditTargetGrid === no ? 'active' : ''}" data-storyboard-grid-btn="${activeIndex}" data-grid-no="${no}" onclick="selectStoryboardEditGrid(${activeIndex}, ${no})">${no}格</button>`;
    }).join('') : '';
    const activeStoryboardEditTools = activeImagePath ? `
        <div class="storyboard-image-edit-tools">
            ${renderStoryboardLightingPresets(activeIndex, activeSeg, { disabled: !activeImagePath })}
            ${renderStoryboardImageVersions(activeIndex, activeSeg)}
            <div class="storyboard-image-edit-path">选择要修改的分镜格；其他格保持不变。</div>
            <div class="storyboard-grid-edit-row">${activeStoryboardGridButtons}</div>
    ...
    const renderCard = (seg, i) => {
        ...
        const imageStatus = imagePath
            ...
            : (seg.storyboard_status === 'generating' && prompt
                    ...
                    <div class="script-prompt-merged-left">
                        ...
                        <div class="script-prompt-editor-wrap">
                            <div class="script-prompt-editor-head">
                                <strong>分镜提示</strong>
                                <span>${activePromptLoading ? '正在生成' : (activePrompt ? `${activePrompt.length}字` : '未生成')}</span>
                            </div>
...
function renderStoryboardImageSidebarItem(seg, i, activeIndex) {
    ...
    const statusText = generating ? (promptReady ? '出图中' : '提示中') : (imageReady ? '已出图' : (promptReady ? '可生成' : '缺 Prompt'));
...
function renderStoryboardImagePage(segments) {
    ...
    const duration = seg.duration || 15;
    const promptLoading = seg.storyboard_status === 'generating' && !prompt;
    const imageLoading = seg.storyboard_status === 'generating' && !!prompt;
    const imageStatus = getStoryboardImageStatus(seg);
    const imagePreview = imagePath
        ? `<img id="storyboard-image-current-${activeIndex}" data-storyboard-current-image="${activeIndex}" src="/${escAttr(imagePath)}" alt="分段 ${activeIndex + 1} 分镜图">`
        : `<div class="storyboard-image-empty">
    ...
    const storyboardEditTools = imagePath ? `
        <div class="storyboard-image-edit-tools">
            ${renderStoryboardPanelTargetButtons(activeIndex, seg, { disabled: !imagePath })}
            ${renderStoryboardLightingPresets(activeIndex, seg, { disabled: !imagePath })}
            ${renderStoryboardImageVersions(activeIndex, seg)}
                ...
                <div class="storyboard-image-workspace">
                    <div class="storyboard-image-prompt-panel">
                        ...
                        <div class="storyboard-image-panel-head">
                            ...
                            <span>${promptLoading ? '正在生成' : (prompt ? `${prompt.length}字` : '未生成')}</span>
                        </div>
                    ...
                    <div class="storyboard-image-preview-panel ${imageLoading ? 'is-loading' : ''}">
                        <div class="storyboard-image-panel-head">
<strong>分镜图</strong>
                            <span>${imagePath ? '图片已生成' : (imageLoading ? '生成中' : '等待生成')}</span>
                        </div>
                        ${imageLoading ? `
                            <div class="storyboard-image-loading">
                                <span class="loading-spinner"></span>
...
function renderVideoPromptPage(segments) {
    ...
    const imagePreview = imagePath
        ? `<img id="video-prompt-storyboard-image-${activeIndex}" data-storyboard-current-image="${activeIndex}" src="/${escAttr(imagePath)}" alt="分段 ${activeIndex + 1} 分镜图">`
        : `<div class="storyboard-image-empty">
                <strong>等待分镜图</strong>
...
function copyStoryboardPromptToVideoColumn(index) {
    ...
    seg.video_prompt_text = text;
    seg._videoPromptDirty = true;
    showToast(`片段 ${index + 1} 已带入视频栏，可继续微调`, 'success');
}

function sanitizeImagePromptForVideoPrompt(imagePrompt = '') {
    let text = String(imagePrompt || '').trim();
    if (!text) return '';
    text = text.split(/负面提示[:：]/)[0].trim();
    const replacements = [
        [/参考图对应：([^。]+)。\s*/g, '参考图对应：$1。\n'],
        [/生成单张[四四六六九十二]+格动态分镜表[，,]?/g, '连续视频镜头参考，'],
        [/\d+\s*:\s*\d+[，,]?/g, ''],
        [/三乘二清晰分格[，,]?/g, ''],
        [/每格内部按16:9构图[，,]?/g, ''],
        [/角落仅保留极小黑色编号\d{2}-\d{2}[；;，,]?/g, ''],
        [/偏古风3D动画预演分镜[，,]?/g, ''],
        [/干净高调黑白灰阶[，,]?/g, ''],
        [/明亮灰白底[，,]?/g, ''],
        [/单色石墨线稿叠加柔和体积明暗[，,]?/g, ''],
...
async function loadSeedanceDialogueSkill() {
    if (seedanceDialogueSkillCache) return seedanceDialogueSkillCache;
    try {
        const res = await fetch(SEEDANCE_DIALOGUE_SKILL_URL, { cache: 'no-store' });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        seedanceDialogueSkillCache = (await res.text()).trim();
    } catch (e) {
        console.warn('读取 Seedance 对话视频 skill 失败，使用最小输出契约兜底', e);
        seedanceDialogueSkillCache = [
    ...
    return seedanceDialogueSkillCache;
}

function buildVideoReferenceTagLine(assetScope = {}) {
    const charTags = (assetScope.characters || [])
        .map(id => {
            const asset = getAssetById('char', id);
            return asset ? `@char:${id}=${asset.name || id}` : '';
    ...
    const sceneTags = (assetScope.scenes || [])
        ...
        .filter(Boolean);
    return charTags.concat(sceneTags).join(' ');
}

function buildVideoSkillAssetMap(assetScope = {}) {
    const lines = [];
    (assetScope.characters || []).forEach(id => {
        const asset = getAssetById('char', id);
        if (asset) lines.push(`@char:${id}=${asset.name || id} -> char:${id}`);
    });
    (assetScope.scenes || []).forEach(id => {
        const asset = getAssetById('scene', id);
        if (asset) lines.push(`@scene:${id}=${asset.name || id} -> scene:${id}`);
    });
    return lines.join('\n');
}

function buildVideoStoryboardReferenceLine(imagePath = '') {
    if (!imagePath) return '';
    return '【图6】=当前片段已生成的分镜图 reference photo / storyboard first-frame lock；生成视频时会作为真实首帧锚定图上传。最终视频提示词必须写明：【图6】作为首帧锚定图，视频第一帧严格匹配【图6】中当前片段起始格或对应视觉区域的构图、人物相对站位、镜头角度、主体比例、前中后景、景深和光影节奏；前0.5-1秒保持锁帧构图，再从该首帧自然运动；不要生成分镜格线、编号、箭头、标注、字幕、对白框或UI。';
}
...
async function buildVideoPromptGenerationInstruction(seg, index, assetScope = {}, imagePath = '') {
    const skillText = await loadSeedanceDialogueSkill();
    const tagLine = buildVideoReferenceTagLine(assetScope);
    const storyboardReferenceLine = buildVideoStoryboardReferenceLine(imagePath || seg?.storyboard_image_path || '');
    return [
        skillText,
        '',
        '【网页插口约束】',
...
function ensureVideoReferenceTags(prompt = '', assetScope = {}, imagePath = '') {
    const text = String(prompt || '').trim();
    if (!text) return text;
    const tagLine = buildVideoReferenceTagLine(assetScope);
    const storyboardReferenceLine = buildVideoStoryboardReferenceLine(imagePath);
    const prefixLines = [];
    if (tagLine) {
        const missing = tagLine.split(/\s+/).filter(tag => tag && !text.includes(tag.split('=')[0]));
        if (missing.length) prefixLines.push(tagLine);
    }
    if (storyboardReferenceLine && !/【图6】|首帧锚定图|first-frame lock|reference photo/i.test(text)) {
        prefixLines.push(storyboardReferenceLine);
    }
    return prefixLines.length ? `${prefixLines.join('\n')}\n${text}` : text;
}

function isTargetVideoPromptFormat(prompt = '', segmentText = '', imagePrompt = '', backboneText = '') {
    const text = String(prompt || '').trim();
...
async function saveVideoPromptEdit(index) {
    ...
    await saveSegmentState(index, { video_prompt_text: prompt });
    const resultEl = document.getElementById('resultContent');
    if (resultEl) resultEl.textContent = currentSegments.map(s => getSegmentVideoPrompt(s)).join('\n\n===VIDEO_SPLIT===\n\n');
...
async function generateVideoPrompt(index, options = {}) {
    ...
    try {
        ...
        const narrativeText = seg._fullNarrative || currentScriptNarrative || loadPipelineSkillOutputs().narrative || segmentText;
        const backboneText = await generateSegmentBackbone(index, segmentText, narrativeText, { silentLoading: true });
        const assetScope = getStoryboardAssetTagScope(backboneText, { segment: seg });
        const currentScript = buildVideoPromptSourceForModel(index, imagePrompt, imagePath, { segmentText, backboneText, assetScope });
        const instruction = [
            await buildVideoPromptGenerationInstruction(seg, index, assetScope, imagePath)
        ].join('\n\n');
        const res = await fetch(`/api/segments/${seg.id}/reprocess-script`, {
            ...
            body: JSON.stringify({
                ...
                storyboard_reference_image: imagePath,
                reference_images: imagePath ? [imagePath] : []
            })
        });
        const data = await res.json();
        if (!res.ok) {
        ...
        const generated = ensureVideoReferenceTags(sanitizeGeneratedVideoPrompt(updatedSegment.prompt_text || updatedSegment.prompt || data.result || ''), assetScope, imagePath);
        if (!generated) {
            seg._videoPromptStatus = 'failed';
            renderSegments(currentSegments);
        ...
        if (!isTargetVideoPromptFormat(generated, segmentText, imagePrompt, backboneText)) {
            seg._videoPromptStatus = 'failed';
            renderSegments(currentSegments);
            if (!options.silentLoading) showToast('Video Prompt 输出不像视频 skill 结果，已停止写入', 'warning');
            return false;
        }
        currentSegments[index] = {
            ...currentSegments[index],
            ...updatedSegment,
            ...getProtectedSegmentSourcePatch(seg, { prompt: segmentText, prompt_text: segmentText, source_script: segmentText, _original: segmentText }),
            video_prompt_text: generated,
            prompt: segmentText,
            prompt_text: segmentText,
            source_script: segmentText,
            _original: segmentText,
            _fullNarrative: narrativeText,
            _backboneCompact: backboneText,
            _backboneSourceText: segmentText,
            storyboard_prompt: imagePrompt,
            storyboard_image_path: imagePath,
            _videoPromptDirty: true,
            _videoPromptStatus: 'ready'
        };
        await saveSegmentState(index, { prompt_text: segmentText, video_prompt_text: generated, storyboard_prompt: imagePrompt, storyboard_status: currentSegments[index].storyboard_status || 'image_ready' });
        const resultEl = document.getElementById('resultContent');
        if (resultEl) resultEl.textContent = currentSegments.map(s => getSegmentVideoPrompt(s)).join('\n\n===VIDEO_SPLIT===\n\n');
        renderSegments(currentSegments);
...
function initActiveSegmentEditors(seg, i) {
    ...
    const editor = attachMentionEditor(root, {
        ...
        onInput: (ed) => {
            seg.video_prompt_text = ed.getValue();
            seg._videoPromptDirty = true;
        },
    ...
    const editorValue = getSegmentVideoPrompt(seg);
...
function getEditProjectKey() {
    return `${EDIT_STUDIO_STORAGE_KEY}:${currentProject || 'draft'}`;
}

function getEditClipStableSegmentIndex(clip, fallbackIndex = 0, segments = currentSegments) {
    const segmentCount = Array.isArray(segments) ? segments.length : 0;
    const maxIndex = Math.max(segmentCount - 1, 0);
    const raw = Number(clip?.segmentIndex);
    if (Number.isFinite(raw) && raw >= 0 && raw <= maxIndex) {
        return Math.floor(raw);
    }
    const idText = String(clip?.id || '');
    const idMatchedIndex = (segments || []).findIndex((seg) => {
        const segId = String(seg?.id || seg?.segment_id || '').trim();
        if (!segId) return false;
        return idText === `clip-${segId}`
            || idText.startsWith(`clip-${segId}-`);
    });
    if (idMatchedIndex >= 0) return idMatchedIndex;
    const numericIndex = Number.isFinite(raw) ? raw : Number(fallbackIndex);
    return Math.max(0, Math.min(maxIndex, Number.isFinite(numericIndex) ? Math.floor(numericIndex) : 0));
}

function getEditClipSegment(clip, segments = currentSegments) {
    const index = getEditClipStableSegmentIndex(clip, 0, segments);
    return segments[index] || {};
}

function getEditClipBaseDuration(clip, segments = currentSegments) {
    const seg = getEditClipSegment(clip, segments);
    return normalizeVideoDuration(clip?.duration || seg.duration || 5);
}

function getEditClipDuration(clip, segments = currentSegments) {
    if (!clip) return 0;
    const baseDuration = getEditClipBaseDuration(clip, segments);
    const trimIn = Math.max(0, Number(clip.trimIn) || 0);
    const trimOut = Math.max(trimIn + 0.1, Number(clip.trimOut) || baseDuration);
    const speed = Math.max(0.25, Number(clip.speed) || 1);
    return Math.max(0.2, (Math.min(trimOut, baseDuration) - Math.min(trimIn, baseDuration - 0.1)) / speed);
}

function formatEditTimelineTime(seconds) {
...
function getEditTimelineStart(clips, clipId, segments = currentSegments) {
    let cursor = 0;
    for (const clip of clips || []) {
        if (clip.id === clipId) return cursor;
        cursor += getEditClipDuration(clip, segments);
    }
    return 0;
}

function buildEditTimelineRuler(totalDuration, pxPerSecond) {
    const safeDuration = Math.max(1, Number(totalDuration) || 1);
    const interval = safeDuration <= 20 ? 2 : (safeDuration <= 60 ? 5 : 10);
    const marks = [];
    for (let time = 0; time <= safeDuration + 0.001; time += interval) {
        marks.push(`<span class="edit-studio-ruler-mark" style="left:${Math.round(time * pxPerSecond)}px">${formatEditTimelineTime(time)}</span>`);
    }
    if (safeDuration % interval > 0.001) {
        marks.push(`<span class="edit-studio-ruler-mark end" style="left:${Math.round(safeDuration * pxPerSecond)}px">${formatEditTimelineTime(safeDuration)}</span>`);
    }
    return marks.join('');
}

function getEditClipDisplayLabel(clip, orderIndex) {
    const suffix = /(?:\s[A-B]|\scopy)$/i.test(String(clip?.label || '')) ? String(clip.label).replace(/^.*?(?:\s|$)([A-B]|copy)$/i, ' $1') : '';
    return `片段 ${getEditClipStableSegmentIndex(clip, orderIndex, currentSegments) + 1}${suffix}`;
}

function sanitizeEditClip(clip, index, segments = currentSegments) {
    const segmentIndex = getEditClipStableSegmentIndex(clip, index, segments);
    const seg = segments[segmentIndex] || {};
    const baseDuration = normalizeVideoDuration(clip?.duration || seg.duration || 5);
    const trimIn = Math.max(0, Math.min(baseDuration - 0.1, Number(clip?.trimIn) || 0));
    const trimOut = Math.max(trimIn + 0.1, Math.min(baseDuration, Number(clip?.trimOut) || baseDuration));
    return {
        id: clip?.id || `clip-${seg.id || segmentIndex}-${index}`,
        segmentIndex,
        label: clip?.label || `片段 ${segmentIndex + 1}`,
        enabled: clip?.enabled !== false,
        trimIn: Number(trimIn.toFixed(2)),
        trimOut: Number(trimOut.toFixed(2)),
        duration: baseDuration,
        volume: Math.max(0, Math.min(200, Number(clip?.volume ?? 100))),
        muted: clip?.muted === true,
        fadeIn: Math.max(0, Math.min(10, Number(clip?.fadeIn) || 0)),
        fadeOut: Math.max(0, Math.min(10, Number(clip?.fadeOut) || 0)),
        transition: clip?.transition || 'cut',
        speed: Math.max(0.25, Math.min(4, Number(clip?.speed) || 1))
    };
}

function normalizeEditAudioTrack(track = {}, type = 'music') {
    const isVoice = type === 'voice';
    return {
        id: track.id || `${type}-track`,
        type,
        name: track.name || (isVoice ? '旁白' : '背景音乐'),
        sourcePath: track.sourcePath || track.path || '',
        start: Math.max(0, Number(track.start) || 0),
        volume: Math.max(0, Math.min(200, Number(track.volume ?? (isVoice ? 100 : 60)))),
        muted: track.muted === true,
        fadeIn: isVoice ? 0 : Math.max(0, Math.min(30, Number(track.fadeIn) || 0)),
        fadeOut: isVoice ? 0 : Math.max(0, Math.min(30, Number(track.fadeOut) || 0)),
        duration: Math.max(0, Number(track.duration) || 0),
        status: track.status || (track.sourcePath || track.path ? 'ready' : 'empty')
    };
}

function normalizeEditStudioState(state, segments = currentSegments) {
    const fallback = getDefaultEditTimeline(segments);
    const clips = Array.isArray(state?.clips) && state.clips.length
        ? state.clips.map((clip, i) => sanitizeEditClip(clip, i, segments))
        : fallback.clips;
    const hasEditOperations = clips.some(clip => /(?:-split-|-copy-|\sA$|\sB$|\scopy$)/i.test(`${clip.id || ''} ${clip.label || ''}`));
    const uniqueSegmentCount = new Set(clips.map(clip => clip.segmentIndex)).size;
    const shouldRepairOneToOneOrder = !hasEditOperations && clips.length === segments.length && uniqueSegmentCount < segments.length;
    const repairedClips = shouldRepairOneToOneOrder
        ? fallback.clips.map((clip, i) => sanitizeEditClip({ ...clips[i], id: clip.id, segmentIndex: i, label: `片段 ${i + 1}` }, i, segments))
        : clips;
    const safeState = {
        ...fallback,
        ...state,
        version: 2,
        clips: repairedClips,
        timelineZoom: Math.max(0.6, Math.min(3, Number(state?.timelineZoom) || 1)),
        playheadSeconds: Math.max(0, Number(state?.playheadSeconds) || 0),
        exportStatus: state?.exportStatus || 'idle',
        exportVideoPath: state?.exportVideoPath || '',
        audioTracks: {
            music: normalizeEditAudioTrack(state?.audioTracks?.music, 'music'),
            voice: normalizeEditAudioTrack(state?.audioTracks?.voice, 'voice')
        }
    };
    const total = getEditTimelineTotalDuration(safeState.clips, segments, false);
    safeState.playheadSeconds = Math.max(0, Math.min(total, safeState.playheadSeconds));
    if (!safeState.clips.some(clip => clip.id === safeState.activeClipId)) {
        safeState.activeClipId = safeState.clips[0]?.id || '';
    }
    return safeState;
}

function getDefaultEditTimeline(segments = currentSegments) {
    const clips = segments.map((seg, i) => {
        const duration = normalizeVideoDuration(seg?.duration || 5);
        return sanitizeEditClip({
            id: `clip-${seg?.id || i}`,
            segmentIndex: i,
            label: `片段 ${i + 1}`,
            trimIn: 0,
    ...
    const activeClip = clips[Math.max(0, Math.min(activeSegmentEditIndex, clips.length - 1))];
    return {
        version: 2,
        projectId: currentProject || 'draft',
        ...
        exportVideoPath: '',
        audioTracks: {
            music: normalizeEditAudioTrack({}, 'music'),
            voice: normalizeEditAudioTrack({}, 'voice')
        },
        clips,
        updatedAt: Date.now()
    };
...
function loadEditStudioState(segments = currentSegments) {
    const fallback = getDefaultEditTimeline(segments);
    let state = fallback;
    try {
        const raw = localStorage.getItem(getEditProjectKey());
        if (raw) {
            const parsed = JSON.parse(raw);
            if (Array.isArray(parsed?.clips) && parsed.clips.length) {
                const clips = parsed.clips
                    .filter((clip, i) => {
                        const index = getEditClipStableSegmentIndex(clip, i, segments);
                        return index >= 0 && index < segments.length;
                    })
                    .map((clip, i) => sanitizeEditClip(clip, i, segments));
                const usedSegmentIndexes = new Set(clips.map(clip => clip.segmentIndex));
                fallback.clips.forEach(clip => {
                    if (!usedSegmentIndexes.has(clip.segmentIndex)) clips.push(clip);
                });
                state = { ...fallback, ...parsed, clips };
            }
        }
    } catch (e) {
        state = fallback;
    }
    state = normalizeEditStudioState(state, segments);
    if (!state.clips.length) state = fallback;
    const activeExists = state.clips.some(clip => clip.id === (editStudioActiveClipId || state.activeClipId));
    state.activeClipId = activeExists ? (editStudioActiveClipId || state.activeClipId) : (state.clips[0]?.id || '');
    editStudioActiveClipId = state.activeClipId;
    return state;
}

function saveEditStudioState(state) {
    if (!state) return;
    const normalized = normalizeEditStudioState(state, currentSegments);
    const safeState = {
        ...normalized,
        activeClipId: normalized.activeClipId || editStudioActiveClipId,
        clips: Array.isArray(normalized.clips) ? normalized.clips.map((clip, i) => sanitizeEditClip(clip, i, currentSegments)) : [],
        updatedAt: Date.now()
    };
    localStorage.setItem(getEditProjectKey(), JSON.stringify(safeState));
    editStudioActiveClipId = safeState.activeClipId || safeState.clips[0]?.id || '';
}

function getActiveEditClip(state) {
    if (!state?.clips?.length) return null;
    return state.clips.find(clip => clip.id === state.activeClipId) || state.clips[0];
}

function getEditTimelineTotalDuration(clips, segments = currentSegments, enabledOnly = true) {
    return (clips || []).reduce((sum, clip) => {
        if (enabledOnly && clip.enabled === false) return sum;
        return sum + getEditClipDuration(clip, segments);
    }, 0);
}

function getEditPixelsPerSecond(state) {
    return Math.round(34 * Math.max(0.6, Math.min(3, Number(state?.timelineZoom) || 1)));
}

function getEditClipAtTimelineTime(state, seconds, segments = currentSegments) {
    let cursor = 0;
    for (const clip of state?.clips || []) {
        const duration = getEditClipDuration(clip, segments);
        if (seconds >= cursor && seconds <= cursor + duration) {
            return { clip, start: cursor, end: cursor + duration, local: seconds - cursor };
        }
        cursor += duration;
    }
    const last = state?.clips?.[state.clips.length - 1] || null;
    const total = getEditTimelineTotalDuration(state?.clips || [], segments, false);
    return last ? { clip: last, start: Math.max(0, total - getEditClipDuration(last, segments)), end: total, local: getEditClipDuration(last, segments) } : null;
}

function setEditStudioPlayhead(seconds, options = {}) {
    const state = loadEditStudioState(currentSegments);
    const total = getEditTimelineTotalDuration(state.clips, currentSegments, false);
    state.playheadSeconds = Math.max(0, Math.min(total, Number(seconds) || 0));
    const hit = getEditClipAtTimelineTime(state, state.playheadSeconds, currentSegments);
    if (hit?.clip) {
        state.activeClipId = hit.clip.id;
        editStudioActiveClipId = hit.clip.id;
        activeSegmentEditIndex = hit.clip.segmentIndex;
    }
    refreshEditStudio(state, options.message || '');
}

function seekEditStudioTimeline(event, lane) {
    if (!lane) return;
    const state = loadEditStudioState(currentSegments);
    const rect = lane.getBoundingClientRect();
    const pxPerSecond = getEditPixelsPerSecond(state);
    const x = Math.max(0, event.clientX - rect.left + lane.scrollLeft);
    setEditStudioPlayhead(x / pxPerSecond);
}

function setEditTimelineZoom(delta) {
    const state = loadEditStudioState(currentSegments);
    state.timelineZoom = Math.max(0.6, Math.min(3, Number(state.timelineZoom || 1) + Number(delta || 0)));
    refreshEditStudio(state);
}

function reorderEditClip(clipId, targetId) {
...
function startEditClipDrag(event, clipId) {
    editStudioDragClipId = clipId;
    event.dataTransfer?.setData('text/plain', clipId);
    event.currentTarget?.classList.add('dragging');
}

function dropEditClip(event, targetId) {
    event.preventDefault();
    const clipId = event.dataTransfer?.getData('text/plain') || editStudioDragClipId;
    editStudioDragClipId = '';
    document.querySelectorAll('.edit-studio-clip.drag-over,.edit-studio-clip.dragging').forEach(el => el.classList.remove('drag-over', 'dragging'));
    reorderEditClip(clipId, targetId);
}

function startEditTrimDrag(event, clipId, side) {
    event.preventDefault();
    event.stopPropagation();
    const state = loadEditStudioState(currentSegments);
    ...
    const startIn = Number(clip.trimIn) || 0;
    const startOut = Number(clip.trimOut) || getEditClipBaseDuration(clip, currentSegments);
    const pxPerSecond = getEditPixelsPerSecond(state);
    const speed = Math.max(0.25, Number(clip.speed) || 1);
    const onMove = (moveEvent) => {
        const deltaSeconds = ((moveEvent.clientX - startX) / pxPerSecond) * speed;
        if (side === 'in') {
            clip.trimIn = Math.max(0, Math.min(startOut - 0.2, startIn + deltaSeconds));
        } else {
            clip.trimOut = Math.max(startIn + 0.2, Math.min(getEditClipBaseDuration(clip, currentSegments), startOut + deltaSeconds));
        }
        saveEditStudioState(state);
        const el = document.querySelector(`[data-edit-clip-id="${CSS.escape(clipId)}"]`);
        if (el) {
            const width = Math.max(34, Math.round(getEditClipDuration(clip, currentSegments) * pxPerSecond));
            el.style.width = `${width}px`;
            const small = el.querySelector('small');
            if (small) small.textContent = `${(clip.trimIn || 0).toFixed(1)}-${(clip.trimOut || 0).toFixed(1)}s`;
        }
    };
    const onUp = () => {
        document.removeEventListener('pointermove', onMove);
        document.removeEventListener('pointerup', onUp);
        refreshEditStudio(loadEditStudioState(currentSegments));
    };
    document.addEventListener('pointermove', onMove);
    document.addEventListener('pointerup', onUp);
}

function refreshEditStudio(state, message = '') {
    saveEditStudioState(state);
    activeProductionFlowStep = 'edit';
    const activeClip = getActiveEditClip(state);
    if (activeClip) activeSegmentEditIndex = activeClip.segmentIndex;
    renderSegments(currentSegments);
    if (message) showToast(message, 'success');
}

function selectEditClip(clipId) {
    const state = loadEditStudioState(currentSegments);
    const clip = state.clips.find(item => item.id === clipId);
    if (!clip) return;
    state.activeClipId = clip.id;
    state.playheadSeconds = getEditTimelineStart(state.clips, clip.id, currentSegments);
    editStudioActiveClipId = clip.id;
    activeSegmentEditIndex = clip.segmentIndex;
    refreshEditStudio(state);
}

function updateEditClipField(clipId, field, value) {
    const state = loadEditStudioState(currentSegments);
    const clip = state.clips.find(item => item.id === clipId);
    if (!clip) return;
    const numericFields = new Set(['trimIn', 'trimOut', 'volume', 'fadeIn', 'fadeOut', 'speed']);
    clip[field] = field === 'muted' ? Boolean(value) : (numericFields.has(field) ? Number(value) : value);
    const safe = sanitizeEditClip(clip, state.clips.indexOf(clip), currentSegments);
    Object.assign(clip, safe);
    state.activeClipId = clip.id;
    refreshEditStudio(state);
}

function moveEditClip(clipId, direction) {
    const state = loadEditStudioState(currentSegments);
...
function splitEditClip(clipId) {
    const state = loadEditStudioState(currentSegments);
    const index = state.clips.findIndex(clip => clip.id === clipId);
    if (index < 0) return;
    const clip = state.clips[index];
    const clipStart = getEditTimelineStart(state.clips, clipId, currentSegments);
    const localTimeline = Math.max(0, (Number(state.playheadSeconds) || clipStart) - clipStart);
    const cutPoint = Number((Number(clip.trimIn) + localTimeline * Math.max(0.25, Number(clip.speed) || 1)).toFixed(2));
    const midpoint = cutPoint > clip.trimIn + 0.1 && cutPoint < clip.trimOut - 0.1
        ? cutPoint
        : Number(((Number(clip.trimIn) + Number(clip.trimOut)) / 2).toFixed(2));
    if (midpoint <= clip.trimIn + 0.1 || midpoint >= clip.trimOut - 0.1) {
        showToast('片段太短，不能继续切开', 'warning');
        return;
    }
    const second = { ...clip, id: `${clip.id}-split-${Date.now()}`, label: `${clip.label} B`, trimIn: midpoint };
    clip.label = `${clip.label} A`;
    clip.trimOut = midpoint;
    state.clips.splice(index + 1, 0, sanitizeEditClip(second, index + 1, currentSegments));
    state.activeClipId = second.id;
    state.playheadSeconds = getEditTimelineStart(state.clips, second.id, currentSegments);
    refreshEditStudio(state, '已拆分片段');
}

function duplicateEditClip(clipId) {
    const state = loadEditStudioState(currentSegments);
    const index = state.clips.findIndex(clip => clip.id === clipId);
    if (index < 0) return;
    const copy = sanitizeEditClip({ ...state.clips[index], id: `${clipId}-copy-${Date.now()}`, label: `${state.clips[index].label} copy` }, index + 1, currentSegments);
    state.clips.splice(index + 1, 0, copy);
    state.activeClipId = copy.id;
    refreshEditStudio(state, '已复制片段');
}

function deleteEditClip(clipId) {
    const state = loadEditStudioState(currentSegments);
    if (state.clips.length <= 1) {
        const onlyClip = state.clips[0];
        if (onlyClip) onlyClip.enabled = false;
    } else {
        state.clips = state.clips.filter(clip => clip.id !== clipId);
    }
    state.activeClipId = state.clips[0]?.id || '';
    refreshEditStudio(state, '已移除片段');
}

function toggleEditClipEnabled(clipId) {
    const state = loadEditStudioState(currentSegments);
    const clip = state.clips.find(item => item.id === clipId);
    if (!clip) return;
...
function resetEditClip(clipId) {
    const state = loadEditStudioState(currentSegments);
    const index = state.clips.findIndex(clip => clip.id === clipId);
    if (index < 0) return;
    const seg = currentSegments[state.clips[index].segmentIndex] || {};
    const duration = normalizeVideoDuration(seg.duration || 5);
    state.clips[index] = sanitizeEditClip({
        id: state.clips[index].id,
        segmentIndex: state.clips[index].segmentIndex,
        label: `片段 ${state.clips[index].segmentIndex + 1}`,
        trimIn: 0,
        trimOut: duration,
        duration
    }, index, currentSegments);
    state.activeClipId = state.clips[index].id;
    refreshEditStudio(state, '已重置当前片段');
}

function saveCurrentEditStudio() {
    const state = loadEditStudioState(currentSegments);
    saveEditStudioState(state);
    showToast('剪辑已保存', 'success');
}

function updateEditAudioTrackField(type, field, value) {
    const state = loadEditStudioState(currentSegments);
    const track = normalizeEditAudioTrack(state.audioTracks?.[type], type);
    const numericFields = new Set(['start', 'volume', 'fadeIn', 'fadeOut', 'duration']);
    track[field] = field === 'muted' ? Boolean(value) : (numericFields.has(field) ? Number(value) : value);
    state.audioTracks = { ...state.audioTracks, [type]: normalizeEditAudioTrack(track, type) };
    refreshEditStudio(state);
}

async function uploadEditAudioTrack(type, input) {
    const file = input?.files?.[0];
    if (!file) return;
    const state = loadEditStudioState(currentSegments);
    state.audioTracks[type] = normalizeEditAudioTrack({
        ...state.audioTracks?.[type],
    ...
    try {
        ...
        const data = await res.json().catch(() => ({}));
        if (!res.ok || !data.video_path) throw new Error(data.error || 'upload failed');
        const next = loadEditStudioState(currentSegments);
        next.audioTracks[type] = normalizeEditAudioTrack({
            ...next.audioTracks?.[type],
            name: file.name.replace(/\.[^.]+$/, ''),
            sourcePath: data.video_path,
            status: 'ready'
        }, type);
        refreshEditStudio(next, type === 'voice' ? '旁白已加入' : '背景音乐已加入');
    } catch (e) {
        const next = loadEditStudioState(currentSegments);
        next.audioTracks[type] = normalizeEditAudioTrack({ ...next.audioTracks?.[type], status: 'failed' }, type);
        refreshEditStudio(next);
        showToast('音频导入失败，请重试', 'error');
    } finally {
        if (input) input.value = '';
    }
}

function clearEditAudioTrack(type) {
    const state = loadEditStudioState(currentSegments);
    state.audioTracks[type] = normalizeEditAudioTrack({}, type);
    refreshEditStudio(state);
}

function updateEditPreviewPlayhead(seconds) {
    const head = document.getElementById('editStudioPlayhead');
    const time = document.getElementById('editStudioPlayheadTime');
    const state = loadEditStudioState(currentSegments);
    const pxPerSecond = getEditPixelsPerSecond(state);
    if (head) head.style.left = `${Math.round(Math.max(0, seconds) * pxPerSecond)}px`;
    if (time) time.textContent = formatEditTimelineTime(seconds);
}

function playEditStudioActiveClip() {
    playEditStudioTimeline();
}

function playEditStudioTimeline() {
    const state = loadEditStudioState(currentSegments);
    const clip = getActiveEditClip(state);
    const video = document.getElementById('editStudioPreviewVideo');
    if (!clip || !video) return;
    const trimIn = Math.max(0, Number(clip.trimIn) || 0);
    const trimOut = Math.max(trimIn + 0.1, Number(clip.trimOut) || getEditClipBaseDuration(clip, currentSegments));
    const clipStart = getEditTimelineStart(state.clips, clip.id, currentSegments);
    const local = Math.max(0, Math.min(getEditClipDuration(clip, currentSegments), (Number(state.playheadSeconds) || clipStart) - clipStart));
    video.currentTime = trimIn;
    if (local > 0) video.currentTime = Math.min(trimOut - 0.05, trimIn + local * Math.max(0.25, Number(clip.speed) || 1));
    video.playbackRate = Math.max(0.25, Number(clip.speed) || 1);
    video.muted = clip.muted === true;
    video.volume = Math.max(0, Math.min(1, Number(clip.volume || 100) / 100));
    if (editStudioPreviewTimer) clearInterval(editStudioPreviewTimer);
    editStudioPreviewTimer = setInterval(() => {
        updateEditPreviewPlayhead(clipStart + Math.max(0, (video.currentTime - trimIn) / Math.max(0.25, Number(clip.speed) || 1)));
    }, 80);
    video.ontimeupdate = () => {
...
function renderVideoEditStudioPageV2(segments) {
    const state = loadEditStudioState(segments);
    const activeClip = getActiveEditClip(state);
    const activeIndex = Math.max(0, Math.min(segments.length - 1, Number(activeClip?.segmentIndex) || 0));
    const seg = segments[activeIndex] || {};
    normalizeSegmentState(seg);
    activeSegmentEditIndex = activeIndex;
    const title = getSegmentStoryTitle(seg, activeIndex);
    ...
    const imagePath = seg.storyboard_image_path || '';
    const duration = getEditClipBaseDuration(activeClip, segments);
    const effectiveDuration = getEditClipDuration(activeClip, segments);
    const totalDuration = getEditTimelineTotalDuration(state.clips, segments, true);
    const timelineDuration = getEditTimelineTotalDuration(state.clips, segments, false);
    const pxPerSecond = getEditPixelsPerSecond(state);
    const timelineWidth = Math.max(720, Math.ceil(Math.max(1, timelineDuration) * pxPerSecond));
    const playheadSeconds = Math.max(0, Math.min(timelineDuration, Number(state.playheadSeconds) || getEditTimelineStart(state.clips, activeClip?.id, segments)));
    const playheadLeft = Math.round(playheadSeconds * pxPerSecond);
    const projectTitle = currentProjectInfo?.name || '未命名项目';
    const activeClipId = escAttr(activeClip?.id || '');
    const markerList = buildEditTimelineRuler(timelineDuration, pxPerSecond);
    const sceneList = state.clips.map((clip, i) => {
        const item = segments[clip.segmentIndex] || {};
        normalizeSegmentState(item);
        const itemVideo = item._localVideoPath ? videoSrcWithCache(item._localVideoPath) : (item._videoUrl || item.video_url || item.local_video_path || '');
        const itemThumb = item.storyboard_image_path
            ? `<img src="/${escAttr(item.storyboard_image_path)}" alt="素材${i + 1}">`
            : (itemVideo ? `<video src="${escAttr(itemVideo)}" muted preload="metadata"></video>` : `<span>${clip.segmentIndex + 1}</span>`);
        return `
            <button type="button" class="edit-studio-scene ${clip.id === activeClip?.id ? 'active' : ''} ${clip.enabled === false ? 'disabled' : ''}" onclick="selectEditClip('${escAttr(clip.id)}')">
                <span class="edit-studio-scene-thumb">${itemThumb}</span>
                <span class="edit-studio-scene-copy"><strong>${escHtml(getEditClipDisplayLabel(clip, i))}</strong><small>${getEditClipDuration(clip, segments).toFixed(1)}s · ${clip.speed || 1}x</small></span>
                <span class="edit-studio-scene-order">${i + 1}</span>
            </button>`;
    }).join('');
    let videoCursor = 0;
    const clipList = state.clips.map((clip, i) => {
        const d = getEditClipDuration(clip, segments);
        const width = Math.max(28, Math.round(d * pxPerSecond));
        const cls = `${clip.id === activeClip?.id ? 'active' : ''} ${clip.enabled === false ? 'disabled' : ''}`;
        const start = videoCursor;
        videoCursor += d;
        return `<button type="button" class="edit-studio-clip ${cls}" data-edit-clip-id="${escAttr(clip.id)}" draggable="true" style="width:${width}px" onclick="selectEditClip('${escAttr(clip.id)}')" ondragstart="startEditClipDrag(event, '${escAttr(clip.id)}')" ondragover="event.preventDefault(); this.classList.add('drag-over')" ondragleave="this.classList.remove('drag-over')" ondrop="dropEditClip(event, '${escAttr(clip.id)}')" title="${formatEditTimelineTime(start)}-${formatEditTimelineTime(videoCursor)}"><span class="edit-studio-trim-handle left" onpointerdown="startEditTrimDrag(event, '${escAttr(clip.id)}', 'in')"></span><strong>${escHtml(getEditClipDisplayLabel(clip, i))}</strong><small>${(clip.trimIn || 0).toFixed(1)}-${(clip.trimOut || duration).toFixed(1)}s</small><span class="edit-studio-trim-handle right" onpointerdown="startEditTrimDrag(event, '${escAttr(clip.id)}', 'out')"></span></button>`;
    }).join('');
    let audioCursor = 0;
    const audioClipList = state.clips.map((clip) => {
        const d = getEditClipDuration(clip, segments);
        const width = Math.max(28, Math.round(d * pxPerSecond));
        const start = audioCursor;
        audioCursor += d;
        return `<button type="button" class="edit-studio-clip ghost ${clip.id === activeClip?.id ? 'active' : ''} ${clip.enabled === false ? 'disabled' : ''}" style="width:${width}px" onclick="selectEditClip('${escAttr(clip.id)}')" title="${formatEditTimelineTime(start)}-${formatEditTimelineTime(audioCursor)}">原声 ${clip.segmentIndex + 1}<small>${clip.muted ? '静音' : `${clip.volume}%`}</small></button>`;
    }).join('');
    const renderAudioTrackClip = (type) => {
        const track = normalizeEditAudioTrack(state.audioTracks?.[type], type);
        if (!track.sourcePath) return `<span class="edit-studio-audio-empty">${type === 'voice' ? '上传旁白后显示在这里' : '上传音乐后显示在这里'}</span>`;
        const start = Math.max(0, Number(track.start) || 0);
        const width = Math.max(90, Math.round(Math.max(3, track.duration || Math.max(3, totalDuration - start)) * pxPerSecond));
        const left = Math.round(start * pxPerSecond);
        return `<button type="button" class="edit-studio-clip audio ${track.muted ? 'disabled' : ''}" style="width:${width}px; margin-left:${left}px" title="${escAttr(track.name)}"><strong>${escHtml(track.name)}</strong><small>${track.muted ? '静音' : `${track.volume}%`} · ${formatEditTimelineTime(start)}</small></button>`;
    };
    const exportStatusText = state.exportStatus === 'processing' ? '处理中'
        : (state.exportStatus === 'ready' && state.exportVideoPath ? '可导出'
        : (state.exportStatus === 'failed' ? '导出失败，请重试' : '可编辑'));
    const exportLink = state.exportVideoPath
        ? `<a class="btn btn-sm btn-secondary" href="${escAttr(videoSrcWithCache(state.exportVideoPath))}" download>下载成片</a>`
        : '';
    const preview = videoSrc
        ? `<video id="editStudioPreviewVideo" src="${escAttr(videoSrc)}" controls preload="metadata"></video>`
        : (imagePath ? `<img src="/${escAttr(imagePath)}" alt="当前分镜图">` : `<div class="edit-studio-empty"><strong>先生成视频</strong><span>03 会使用已生成片段做剪辑</span></div>`);
    return `
        <div class="edit-studio-page">
            <aside class="edit-studio-scenes video-editor-media-panel">
                <div class="edit-studio-panel-head"><strong>素材 / 顺序</strong><button type="button" onclick="saveCurrentEditStudio()">保存</button></div>
                <div class="edit-studio-scene-list">${sceneList}</div>
            </aside>
            <section class="edit-studio-main">
                <div class="edit-studio-project-bar">
                    <div><strong>${escHtml(projectTitle)}</strong><small>03 编辑 · ${state.clips.length} 个片段 · 总时长 ${formatEditTimelineTime(totalDuration)} · ${exportStatusText}</small></div>
                    <div class="edit-studio-toolbar">
                        <button class="btn btn-sm btn-secondary" onclick="setEditTimelineZoom(-0.2)">缩小</button>
                        <button class="btn btn-sm btn-secondary" onclick="setEditTimelineZoom(0.2)">放大</button>
                        <button class="btn btn-sm btn-secondary" onclick="saveCurrentEditStudio()">保存</button>
                        ${exportLink}
                        <button class="btn btn-sm btn-primary" onclick="mergeProjectVideos()">导出视频</button>
                    </div>
                </div>
                <div class="edit-studio-preview">
                    <span class="edit-studio-zoom">${effectiveDuration.toFixed(1)}s</span>
                ...
                <div class="edit-studio-playbar">
                    <button type="button" onclick="playEditStudioTimeline()">▶</button>
                    <span><b id="editStudioPlayheadTime">${formatEditTimelineTime(playheadSeconds)}</b> · 入点 ${(activeClip?.trimIn || 0).toFixed(1)}s / 出点 ${(activeClip?.trimOut || duration).toFixed(1)}s</span>
                    <button type="button" onclick="splitEditClip('${activeClipId}')">拆分</button>
                </div>
                <div class="edit-studio-timeline">
                    <div class="edit-studio-ruler"><span></span><div onclick="seekEditStudioTimeline(event, this)" style="width:${timelineWidth}px; --playhead-left:${playheadLeft}px">${markerList}<i id="editStudioPlayhead" class="edit-studio-playhead"></i></div></div>
                    <div class="edit-studio-track"><span class="edit-studio-track-label">视频</span><div style="width:${timelineWidth}px">${clipList}</div></div>
                    <div class="edit-studio-track"><span class="edit-studio-track-label">原声</span><div style="width:${timelineWidth}px">${audioClipList}</div></div>
                    <div class="edit-studio-track"><span class="edit-studio-track-label">音乐</span><div class="edit-studio-audio-lane" style="width:${timelineWidth}px">${renderAudioTrackClip('music')}</div></div>
                    <div class="edit-studio-track"><span class="edit-studio-track-label">旁白</span><div class="edit-studio-audio-lane" style="width:${timelineWidth}px">${renderAudioTrackClip('voice')}</div></div>
                </div>
            </section>
            <aside class="edit-studio-settings video-editor-property-panel">
                <div class="edit-studio-panel-head"><strong>剪辑参数</strong><button type="button" onclick="resetEditClip('${activeClipId}')">重置</button></div>
                <div class="figure-setting-section">
                    <strong>片段操作</strong>
                    <div class="edit-studio-clip-actions">
                        <button type="button" onclick="moveEditClip('${activeClipId}', -1)">前移</button>
                        <button type="button" onclick="moveEditClip('${activeClipId}', 1)">后移</button>
                        <button type="button" onclick="splitEditClip('${activeClipId}')">拆分</button>
                        <button type="button" onclick="duplicateEditClip('${activeClipId}')">复制</button>
                        <button type="button" class="danger" onclick="deleteEditClip('${activeClipId}')">删除</button>
                    </div>
                </div>
                <div class="figure-setting-section">
                    <strong>裁切 / 声音</strong>
                    <div class="edit-studio-trim-grid">
                        <label>开始<input type="number" min="0" max="${duration}" step="0.1" value="${activeClip?.trimIn ?? 0}" onchange="updateEditClipField('${activeClipId}', 'trimIn', this.value)"></label>
                        <label>结束<input type="number" min="0.1" max="${duration}" step="0.1" value="${activeClip?.trimOut ?? duration}" onchange="updateEditClipField('${activeClipId}', 'trimOut', this.value)"></label>
                        <label>原声音量<input type="number" min="0" max="200" step="1" value="${activeClip?.volume ?? 100}" onchange="updateEditClipField('${activeClipId}', 'volume', this.value)"></label>
                        <label>速度<select onchange="updateEditClipField('${activeClipId}', 'speed', this.value)">
                            ${[0.5, 1, 1.5, 2].map(rate => `<option value="${rate}" ${Number(activeClip?.speed || 1) === rate ? 'selected' : ''}>${rate}x</option>`).join('')}
                        </select></label>
                    </div>
                    <label class="edit-studio-check"><input type="checkbox" ${activeClip?.muted ? 'checked' : ''} onchange="updateEditClipField('${activeClipId}', 'muted', this.checked)"> 静音当前片段原声</label>
                </div>
                <div class="figure-setting-section edit-studio-audio-controls">
                    <strong>背景音乐</strong>
                    <input type="file" accept="audio/*,video/*" onchange="uploadEditAudioTrack('music', this)">
                    <div class="edit-studio-trim-grid">
                        <label>开始<input type="number" min="0" step="0.1" value="${state.audioTracks.music.start}" onchange="updateEditAudioTrackField('music', 'start', this.value)"></label>
                        <label>音量<input type="number" min="0" max="200" step="1" value="${state.audioTracks.music.volume}" onchange="updateEditAudioTrackField('music', 'volume', this.value)"></label>
                        <label>淡入<input type="number" min="0" max="30" step="0.1" value="${state.audioTracks.music.fadeIn}" onchange="updateEditAudioTrackField('music', 'fadeIn', this.value)"></label>
                        <label>淡出<input type="number" min="0" max="30" step="0.1" value="${state.audioTracks.music.fadeOut}" onchange="updateEditAudioTrackField('music', 'fadeOut', this.value)"></label>
                    </div>
                    <div class="edit-studio-inline-row"><label class="edit-studio-check"><input type="checkbox" ${state.audioTracks.music.muted ? 'checked' : ''} onchange="updateEditAudioTrackField('music', 'muted', this.checked)"> 静音音乐</label><button type="button" onclick="clearEditAudioTrack('music')">移除</button></div>
                </div>
                <div class="figure-setting-section edit-studio-audio-controls">
                    <strong>旁白</strong>
                    <input type="file" accept="audio/*,video/*" onchange="uploadEditAudioTrack('voice', this)">
                    <div class="edit-studio-trim-grid">
                        <label>开始<input type="number" min="0" step="0.1" value="${state.audioTracks.voice.start}" onchange="updateEditAudioTrackField('voice', 'start', this.value)"></label>
                        <label>音量<input type="number" min="0" max="200" step="1" value="${state.audioTracks.voice.volume}" onchange="updateEditAudioTrackField('voice', 'volume', this.value)"></label>
                    </div>
                    <div class="edit-studio-inline-row"><label class="edit-studio-check"><input type="checkbox" ${state.audioTracks.voice.muted ? 'checked' : ''} onchange="updateEditAudioTrackField('voice', 'muted', this.checked)"> 静音旁白</label><button type="button" onclick="clearEditAudioTrack('voice')">移除</button></div>
                </div>
                <div class="edit-studio-meta"><strong>${escHtml(title)}</strong><span>原片 ${duration}s · 剪后 ${effectiveDuration.toFixed(1)}s · 设置会自动保存</span></div>
            </aside>
        </div>`;
}

function renderVideoEditStudioPage(segments) {
    return renderVideoEditStudioPageV2(segments);
    const activeIndex = clampActiveSegmentEditIndex(segments);
    const seg = segments[activeIndex] || {};
    normalizeSegmentState(seg);
    const title = getSegmentStoryTitle(seg, activeIndex);
    ...
    const sceneList = segments.map((item, i) => {
        ...
        return `
            <button type="button" class="edit-studio-scene ${i === activeIndex ? 'active' : ''}" onclick="activeSegmentEditIndex=${i}; activeProductionFlowStep='edit'; renderSegments(currentSegments);">
                <span class="edit-studio-scene-thumb">${itemThumb}</span>
                <span class="edit-studio-scene-copy"><strong>素材 ${i + 1}</strong><small>${normalizeVideoDuration(item.duration || 5)}s</small></span>
            </button>`;
    }).join('');
    ...
    return `
        <div class="edit-studio-page">
            <aside class="edit-studio-scenes video-editor-media-panel">
                <div class="edit-studio-panel-head"><strong>素材</strong><button type="button" onclick="showToast('素材添加会在后续接入', 'info')">+</button></div>
                <div class="edit-studio-scene-list">${sceneList}</div>
            </aside>
            <section class="edit-studio-main">
                <div class="edit-studio-project-bar">
                    <div><strong>${escHtml(projectTitle || 'Untitled')}</strong><small>视频编辑 · 场景 ${activeIndex + 1} / ${segments.length}</small></div>
                    <div><button class="btn btn-sm btn-secondary" onclick="mergeProjectVideos()">Share</button><button class="btn btn-sm btn-primary" onclick="mergeProjectVideos()">Publish</button></div>
                </div>
                <div class="edit-studio-preview">
                    <span class="edit-studio-zoom">50%</span>
                    <button type="button" class="edit-studio-fullscreen" onclick="document.querySelector('.edit-studio-preview video, .edit-studio-preview img')?.requestFullscreen?.()">全屏</button>
                    ${preview}
                </div>
                ...
                <div class="edit-studio-timeline">
                    ...
                    <div class="edit-studio-track"><span>Video</span><div>${clipList}</div></div>
                    <div class="edit-studio-track"><span>Voice</span><div>${segments.map((item, i) => `<button class="edit-studio-clip ghost ${i===activeIndex?'active':''}" onclick="activeSegmentEditIndex=${i}; activeProductionFlowStep='edit'; renderSegments(currentSegments);">Voice ${i+1}</button>`).join('')}</div></div>
                </div>
            </section>
            <aside class="edit-studio-settings video-editor-property-panel">
                <div class="edit-studio-panel-head"><strong>视频编辑</strong><button type="button" onclick="showToast('已保持当前参数', 'info')">重置</button></div>
                <div class="figure-setting-section">
                    <strong>轨道</strong>
                    <label>视频轨<select><option>Video</option><option>Overlay</option><option>Effect</option></select></label>
                    <label>音频轨<select><option>Voice</option><option>BGM</option><option>SFX</option></select></label>
                </div>
                <div class="figure-setting-section">
                    <strong>剪辑</strong>
                    <label>入点<input type="number" min="0" step="0.1" value="0"></label>
                    <label>出点<input type="number" min="0" step="0.1" value="${duration}"></label>
                    <label>音量<input type="number" min="0" max="200" step="1" value="100"></label>
                </div>
                <label>提示词<textarea rows="7" readonly>${escHtml(promptText || '先在 02 生成视频提示')}</textarea></label>
...
async function reprocessSingleSegment(index) {
    ...
    try {
        ...
        const updatedVideoPrompt = updatedSegment.video_prompt_text || updatedSegment.video_prompt || '';
        const legacyVideoPrompt = updatedSegment.prompt_text || updatedSegment.prompt || data.result || '';
        currentSegments[index].video_prompt_text = updatedVideoPrompt || (isGeneratedVideoPromptText(legacyVideoPrompt, currentSegments[index]) ? legacyVideoPrompt : currentSegments[index].video_prompt_text || '');
        currentSegments[index].prompt = getSegmentSourceText(currentSegments[index]);
        ...
        document.getElementById('resultContent').textContent = currentSegments.map(s => getSegmentVideoPrompt(s)).join('\n\n===VIDEO_SPLIT===\n\n');
        renderSegments(currentSegments);
        syncStoryboardPromptToggle();
...
function normalizeSegmentState(seg) {
    if (!seg.prompt && seg.prompt_text && !isGeneratedBackboneText(seg.prompt_text) && !isGeneratedStoryboardPromptText(seg.prompt_text) && !isGeneratedVideoPromptText(seg.prompt_text, seg)) seg.prompt = seg.prompt_text;
    if (!seg._original) {
        const initialPrompt = seg.prompt || seg.prompt_text || '';
        seg._original = sanitizeSegmentSourceText(seg.source_script || initialPrompt, seg);
    }
    const displacedStoryboardPrompt = [seg.storyboard_prompt, seg.prompt_text, seg.prompt]
        .map(item => String(item || '').trim())
        .find(item => item && isGeneratedStoryboardPromptText(item) && !isLegacyStoryboardPrompt(item)) || '';
    const stableOriginal = getStableSegmentOriginal(seg);
    if (stableOriginal) {
        seg._original = stableOriginal;
        seg.source_script = stableOriginal;
        if (isGeneratedBackboneText(seg.prompt_text) || isGeneratedStoryboardPromptText(seg.prompt_text) || isGeneratedVideoPromptRaw(seg.prompt_text)) seg.prompt_text = stableOriginal;
        if (isGeneratedBackboneText(seg.prompt) || isGeneratedStoryboardPromptText(seg.prompt) || isGeneratedVideoPromptRaw(seg.prompt)) seg.prompt = stableOriginal;
    }
    const explicitVideoPrompt = (seg.video_prompt_text || seg.video_prompt || '').trim();
    if (explicitVideoPrompt && isLikelySourceScriptText(explicitVideoPrompt, seg)) {
        seg.video_prompt_text = '';
        if (isLikelySourceScriptText(seg.video_prompt || '', seg)) seg.video_prompt = '';
        seg._videoPromptDirty = false;
    }
    if (!seg._videoPromptDirty && isGeneratedVideoPromptText(seg.prompt || seg.prompt_text || '', seg)) {
        seg.video_prompt_text = seg.video_prompt_text || seg.prompt || seg.prompt_text || '';
        seg.prompt = seg._original || seg.source_script || '';
        seg.prompt_text = seg._original || seg.source_script || '';
        seg._videoPromptDirty = true;
    }
    if (!seg.storyboard_prompt) {
        const storyboardCandidate = displacedStoryboardPrompt || [seg.prompt_text, seg.prompt]
            .map(item => String(item || '').trim())
            .find(item => item && isGeneratedStoryboardPromptText(item) && !isLegacyStoryboardPrompt(item));
        if (storyboardCandidate) seg.storyboard_prompt = storyboardCandidate;
    }
...
async function saveSegmentState(index, extra = {}, options = {}) {
    ...
    const editor = document.getElementById(`segEditor-${index}`);
    const durationInput = document.getElementById(`segDuration-${index}`);
    const editorPrompt = (editor ? readMentionEditorValue(editor) : '').trim();
    if (editorPrompt && !isLikelySourceScriptText(editorPrompt, seg)) {
        seg.video_prompt_text = editorPrompt;
        seg._videoPromptDirty = true;
    }
    const prompt = sanitizeSegmentSourceText(seg._original || seg.source_script || seg.prompt_text || seg.prompt || '', seg);
    if (prompt) {
        seg.prompt = prompt;
        seg.prompt_text = prompt;
        seg.source_script = seg.source_script || prompt;
        seg._original = seg._original || prompt;
    }
...
function mergeStoryboardFrameSegment(index, incomingSegment, framePosition, fallbackSeg = {}) {
    ...
    const protectedSource = getProtectedSegmentSourcePatch(latest, fallbackSeg);

    // 后端返回的是“整行片段”，这里只接收本次分镜任务需要的字段；
    // prompt/source_script 永远以页面当前原台本为准，避免模型输入、主干或 Image Prompt 覆盖原文。
    const commonFields = [
        'id', 'project_id', 'segment_index', 'scene_id', 'character_ids',
    ...
    Object.assign(merged, protectedSource);

    currentSegments[index] = merged;
    normalizeSegmentState(currentSegments[index]);
    const rememberedPath = isEnd ? currentSegments[index].end_storyboard_image_path : currentSegments[index].storyboard_image_path;
    if (!isEnd && rememberedPath) rememberStoryboardImageVersion(index, rememberedPath, { created_at: incoming.updated_at || incoming.created_at });
...
async function generateStoryboardPrompt(index, options = {}) {
    const seg = currentSegments[index];
    if (!seg?.id) {
        showToast('请先生成并保存片段流水线内容后再生成 Image Prompt；已禁止旧本地模板兜底。', 'warning');
        return false;
    }
    const editor = document.getElementById(`segEditor-${index}`);
    if (editor) {
        seg.video_prompt_text = readMentionEditorValue(editor).trim();
        seg._videoPromptDirty = true;
    ...
    try {
        ...
        const narrativeText = seg._fullNarrative || currentScriptNarrative || segmentText;
        const backboneText = await generateSegmentBackbone(index, segmentText, narrativeText, options);
        const assetScope = getStoryboardAssetTagScope(`${segmentText}\n${backboneText}`, { segment: seg });
        const storyboardInputText = buildDialogueStoryboardPromptInput({ segmentText, backboneText, assetScope });
        const scopedInstruction = [
            await buildDialogueStoryboardPromptInstruction(),
        ...
        const res = await fetch(`/api/segments/${seg.id}/reprocess-script`, {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                current_script: storyboardInputText,
                scenes: [],
                characters: [],
                director_mode_id: '',
        ...
        if ((!updatedPrompt || !isTargetStoryboardPromptFormat(updatedPrompt)) && !options.retryCleanFormat) {
            console.warn('Image Prompt 返回内容不符合当前 skill 插口，自动重试一次。', { index, updatedPrompt });
            return generateStoryboardPrompt(index, { ...options, retryCleanFormat: true });
        }
        if (!updatedPrompt || !isTargetStoryboardPromptFormat(updatedPrompt)) {
            seg.storyboard_prompt = oldStoryboardPrompt;
            seg.storyboard_status = oldStoryboardPrompt ? 'prompt_ready' : 'failed';
            seg.continuity_notes = '模型输出不符合当前分镜 skill，已拒绝旧模板兜底。';
            renderSegments(currentSegments);
            renderStoryboardOnlyList(currentSegments);
            if (!options.silentLoading) showToast(`片段 ${index + 1} Image Prompt 输出不符合 skill，已停止写入`, 'warning');
            return false;
        }
        currentSegments[index] = {
            ...seg,
        ...
        seg.storyboard_prompt = oldStoryboardPrompt;
        seg.storyboard_status = oldStoryboardPrompt ? 'prompt_ready' : 'failed';
        seg.continuity_notes = 'Image Prompt 请求失败，已拒绝使用旧网页本地模板兜底: ' + e.message;
        await saveSegmentState(index, {
            prompt_text: segmentText,
            storyboard_prompt: seg.storyboard_prompt,
            storyboard_status: seg.storyboard_status,
            continuity_notes: seg.continuity_notes
        }, { skipStoryboardPrompt: true });
        renderSegments(currentSegments);
        renderStoryboardOnlyList(currentSegments);
        showToast('Image Prompt 请求失败，未写入旧模板: ' + e.message, 'error');
        return false;
    } finally {
...
function getMainlineStoryboardPromptInstruction() {
    return [
        '使用当前 dialogue-storyboard-prompt skill。当前请求只为一个已切分好的短段落生成一条 Image 2 分镜图提示词。',
        '只允许使用【当前段落】和对应 script-backbone-compact 主干；不要混入原始全台本、人物库、场景库、导演模式、视频提示词、网页旧模板或本地兜底模板。',
        '格数、画幅、视觉风格、角色锁、背景锁、逐格镜头和负面提示都按当前 skill 判断；不要固定套六格。',
        '最终只输出一整条自然中文成品 Image Prompt；不要分析、解释、Markdown 标题、网页字段、原台词复述、对白框、字幕或旧模板标题。'
    ].join('\n');
}
...
async function submitStoryboardPromptJob(fixedInstruction, progressKey) {
    ...
    const socketInstruction = await buildDialogueStoryboardPromptInstruction();
...
function inferStoryboardGridCount(seg = {}) {
    const prompt = String(seg.storyboard_prompt || '');
    if (/六格|01[\s\S]*02[\s\S]*03[\s\S]*04[\s\S]*05[\s\S]*06/.test(prompt)) return 6;
    const matches = prompt.match(/(?:^|[^0-9])0?([1-9])(?:\s|，|、|:|：)/g) || [];
    const nums = matches.map(x => Number((x.match(/[1-9]/) || ['0'])[0])).filter(n => n > 0 && n <= 12);
    return nums.length ? Math.max(...nums) : 6;
}

function selectStoryboardEditGrid(index, gridNo) {
    const seg = currentSegments[index];
    if (!seg) return;
    const no = Math.max(1, Number(gridNo) || 1);
    seg._storyboardEditTargetGrid = no;
    const text = `只修改第${no}格，保持其他所有分镜格完全不变。请根据下方修改要求重画第${no}格：`;
    seg._storyboardEditPrompt = text;
    const editor = document.getElementById(`storyboardEditPrompt-${index}`);
    if (editor) editor.value = text;
    document.querySelectorAll(`[data-storyboard-grid-btn="${index}"]`).forEach(btn => btn.classList.toggle('active', Number(btn.dataset.gridNo) === no));
    showToast(`已选择修改第 ${no} 格`, 'info');
}
...
async function editStoryboardImage(index) {
    ...
    try {
        ...
        formData.append('source_image_path', seg.storyboard_image_path);
        ...
        formData.append('reference_image_labels', '当前分镜图 reference photo');
...
function buildMainlineVideoGenerationPrompt(seg, index, imagePrompt, manualVideoPrompt = '') {
    const duration = normalizeVideoDuration(seg?.duration || 5);
    const videoSourcePrompt = sanitizeImagePromptForVideoPrompt(imagePrompt);
    ...
    return [
        ...
        '只继承角色、构图、动作、镜头顺序和空间关系；禁止继承或输出参考图媒介风格词：黑白、灰阶、铅笔、石墨、线稿、分镜表、格线、编号、箭头、timing marks。除非用户明确要求黑白，否则视频按正常成片影像处理。',
        manual ? `用户手动补充的 Video Prompt/视频要求：\n${manual}` : '',
        `Image Prompt 去风格化后的镜头信息：\n${videoSourcePrompt || imagePrompt}`
    ].filter(Boolean).join('\n\n');
}
...
async function generateSingleVideo(index, options = {}) {
    ...
    const manualVideoPrompt = seg._videoPromptDirty && editorPrompt && !isLikelySourceScriptText(editorPrompt, seg) ? editorPrompt : '';
    let savedVideoPrompt = getSegmentVideoPrompt(seg);
    if (mainlineVideoFlow && !manualVideoPrompt && !savedVideoPrompt) {
        const generated = await generateVideoPrompt(index, { silentLoading: true });
        if (generated) {
            const refreshedSeg = currentSegments[index] || seg;
            savedVideoPrompt = getSegmentVideoPrompt(refreshedSeg);
        }
    }
    let prompt = mainlineVideoFlow
        ? (manualVideoPrompt || savedVideoPrompt)
        : editorPrompt;
...
async function importMergedVideoToEditor() {
    ...
    try {
        const editTimeline = loadEditStudioState(currentSegments);
        saveEditStudioState(editTimeline);
        const res = await fetch(`/api/projects/${currentProject}/merge-videos`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ edit_timeline: editTimeline })
        });
        const data = await res.json();
        if (!res.ok) {
...
async function mergeProjectVideos() {
    ...
    const editTimeline = loadEditStudioState(currentSegments);
    editTimeline.exportStatus = 'processing';
    saveEditStudioState(editTimeline);
    showLoading('处理中...', 60);
    try {
        const res = await fetch(`/api/projects/${currentProject}/merge-videos`, {
        ...
        const data = await res.json();
        if (!res.ok) {
            const failed = loadEditStudioState(currentSegments);
            failed.exportStatus = 'failed';
            saveEditStudioState(failed);
            showToast(data.error || '导出失败，请重试', 'error');
            return;
        }
        renderMergedVideo(data.video_path);
        const ready = loadEditStudioState(currentSegments);
        ready.exportStatus = 'ready';
        ready.exportVideoPath = data.video_path || '';
        saveEditStudioState(ready);
        const skippedCount = data.skipped_count || 0;
        const skippedText = skippedCount ? `，已跳过 ${skippedCount} 个未生成完成片段` : '';
        showToast(`导出完成${skippedText}`, 'success');
    } catch (e) {
        const failed = loadEditStudioState(currentSegments);
        failed.exportStatus = 'failed';
        saveEditStudioState(failed);
...
function openDraftProject(project) {
    ...
    switchToTab('script');
    document.getElementById('scriptInput').value = sanitizeScriptInputDisplayText(project?.script_text || '');
    refreshPromptMentionBadges();
...
async function viewProject(projectId, options = {}) {
    try {
        ...
        currentScriptNarrative = project?.narrative_script || '';
        currentSegments = segments.map(s => ({
            ...s,
            prompt: sanitizeSegmentSourceText(s.source_script || s.prompt_text || s.prompt || '', s),
            prompt_text: sanitizeSegmentSourceText(s.source_script || s.prompt_text || s.prompt || '', s),
            source_script: sanitizeSegmentSourceText(s.source_script || s.prompt_text || s.prompt || '', s),
            _original: sanitizeSegmentSourceText(s.source_script || s.prompt_text || s.prompt || '', s),
            storyboard_prompt: s.storyboard_prompt || (isGeneratedStoryboardPromptText(s.prompt_text || s.prompt || '') && !isLegacyStoryboardPrompt(s.prompt_text || s.prompt || '') ? (s.prompt_text || s.prompt || '') : ''),
            _fullNarrative: currentScriptNarrative,
            _backboneCompact: '',
            video_prompt_text: ((s.video_prompt_text || s.video_prompt || '') && !isLikelySourceScriptText(s.video_prompt_text || s.video_prompt || '', s)) ? (s.video_prompt_text || s.video_prompt || '') : (isGeneratedVideoPromptText(s.prompt_text || s.prompt || '', s) ? (s.prompt_text || s.prompt || '') : ''),
            duration: Number(s.duration || 5),
            storyboard_count: Number(s.storyboard_count || 0)
        }));
        hydratePipelineSkillOutputs(currentSegments);
        activeSegmentEditIndex = 0;
        ...
        if (toggle) toggle.checked = localStorage.getItem(STORYBOARD_PROMPTS_STORAGE_KEY) !== '0';
        syncStoryboardPromptToggle();
        document.getElementById('resultContent').textContent = currentSegments.map(s => getSegmentSourceText(s)).join('\n\n');

...
function buildAssetScopeInstruction(scope, label = '当前片段') {
    ...
    return [
        ...
        '最终 Image Prompt 必须显式保留素材标签：有人物素材时写“人物Tag：#角色名”；有场景素材时写“场景Tag：#场景名”，并且只写当前片段实际出现的人物/场景。',
        '最终 Image Prompt 必须是一条自然中文成品提示词：开头可先写“参考图对应”“人物Tag”“场景Tag”，再写“生成单张六格动态分镜表...”，中段用“根据以下剧情转化镜头，不复述台词：...”概括剧情，再写“角色锁：...”“背景锁：...”“01...06...”“负面提示：...”。',
...
function buildAssetTagLine(assetScope = {}) {
    ...
    return [
        charNames.length ? `人物Tag：${charNames.map(name => `#${name}`).join(' ')}` : '',
        sceneNames.length ? `场景Tag：${sceneNames.map(name => `#${name}`).join(' ')}` : ''
...
function ensureStoryboardAssetTags(prompt = '', assetScope = {}) {
    ...
    const charNames = getAssetNamesByIds('char', assetScope.characters);
    const sceneNames = getAssetNamesByIds('scene', assetScope.scenes);
    const needsCharTag = (assetScope.characters || []).length && !/人物Tag[:：]/.test(text);
    const needsSceneTag = (assetScope.scenes || []).length && !/场景Tag[:：]/.test(text);
    if (!needsCharTag && !needsSceneTag) return text;
    const tagLine = [
        needsCharTag && charNames.length ? `人物Tag：${charNames.map(name => `#${name}`).join(' ')}` : '',
        needsSceneTag && sceneNames.length ? `场景Tag：${sceneNames.map(name => `#${name}`).join(' ')}` : ''
    ].filter(Boolean).join('。') + '。';
...
function buildLocalStoryboardPrompt(index, segmentText = '', assetScope = {}) {
    ...
    const assetTagLine = buildAssetTagLine(assetScope);
...
function buildScriptToNarrativeInstruction(extraInstruction = '') {
    const base = [
        '执行 script-to-narrative skill：把完整台本表格/台本输入转成连续 detailed writing 剧本。',
        '读取并融合 Text Window、Context Description、Character 三类信息；谁、说什么、语境动作必须合并成连续动作描写。',
        '补充语境暗示但未写明的物理细节：重心转移、呼吸变化、视线方向、接触点质感、衣物惯性、声音、光线、触感和空间关系。',
        '行与行之间必须用“随即 / 趁 / 与此同时 / 接着”等承接动作桥接，禁止跳跃，禁止遗漏任何语境、动作、QTE。',
        '台词必须融入动作描写，不要单独列对白；可以写“某人的声音紧绷地压进风声里”，不要写成对白清单。',
        '每段末尾加一句简短跨段衔接，暗示下一段第一个事件。',
        '只输出剧本 detailed writing，不输出分析、解释、Markdown 标题、表格或下一步指引。'
    ].join('\n');
    const extra = String(extraInstruction || '').trim();
    return extra ? `${base}\n\n用户额外要求：\n${extra}` : base;
}

function buildScriptBackboneCompactInstruction(index, segmentText, narrativeText) {
    return [
        '执行 script-backbone-compact skill：基于“分段原文 + 剧本详细版”提炼当前单段迷你主干。',
        '只处理当前分段，不要输出其他分段，不要重新分段。',
        '全部使用短语/关键词，不写完整句子。',
        '输出格式必须严格如下：',
        `---分段${index + 1}---`,
...
async function generateScriptNarrative(script, extraInstruction = '', progressKey = '') {
    return requestPipelineText(
        script,
        buildScriptToNarrativeInstruction(extraInstruction),
        progressKey,
...
function hydratePipelineSkillOutputs(segments = currentSegments) {
    ...
    segments.forEach((seg, index) => {
        ...
        if (!seg._backboneCompact && outputs.backbones[key]) seg._backboneCompact = outputs.backbones[key];
    });
}

async function generateSegmentBackbone(index, segmentText, narrativeText, options = {}) {
    const seg = currentSegments[index];
    if (seg?._backboneCompact) return seg._backboneCompact;
    ...
    try {
        const backbone = await requestPipelineText(
            ...
            buildScriptBackboneCompactInstruction(index, source, narrativeText || currentScriptNarrative || source),
            '',
            `script-backbone-compact 正在提炼分段 ${index + 1}`
        );
        if (seg) seg._backboneCompact = backbone;
        if (seg && backbone) {
            savePipelineSkillOutputs({ backbones: { [getSegmentBackboneKey(seg, index)]: backbone } });
...
function isGeneratedBackboneText(text = '') {
    ...
    return /script-backbone-compact|分段主干|剧情主干|主干提炼|人物-动作链|人物动作链|场景锚点|连续性锚点|镜头主干|核心事件|情绪转折|^\s*-{2,}\s*分段\s*\d+/i.test(value);
}
...
function isGeneratedStoryboardPromptText(text = '') {
    ...
    return isLegacyStoryboardPrompt(value)
        ...
        || /参考图对应|人物Tag|场景Tag|生成单张.{0,20}分镜表|角色锁|背景锁|负面提示|镜头路径箭头|三乘二清晰分格|01[：:，,].{0,80}02[：:，,]/.test(value);
}
...
function normalizeStoryboardImageVersions(seg = {}, index = 0) {
    ...
    seg._activeStoryboardImageVersionPath = seg.storyboard_image_path || seg._activeStoryboardImageVersionPath || '';
    try {
    ...
    return `
        <div class="storyboard-image-version-history">
            ...
            <div class="storyboard-image-version-row">
                ${versions.map(item => `
                    <button type="button" class="storyboard-image-version-card ${item.path === activePath ? 'active' : ''}" onclick="selectStoryboardImageVersion(${index}, '${escAttr(item.id)}')">
...
async function selectStoryboardImageVersion(index, versionId) {
    ...
    seg.continuity_notes = `已切换到${version.label}。`;
    ...
    showToast(`已切换到${version.label}`, 'success');
}
...
async function applyStoryboardLightingPreset(index, presetId) {
    ...
    seg._storyboardEditPrompt = preset.prompt;
    const editor = document.getElementById(`storyboardEditPrompt-${index}`);
    if (editor) editor.value = preset.prompt;
    renderSegments(currentSegments);
    renderStoryboardOnlyList(currentSegments);
...
function buildVideoPromptSourceForModel(index, imagePrompt, imagePath) {
    ...
    return [
        `当前片段：分段 ${index + 1}`,
        '输入一：Image Prompt 去风格化后的镜头/动作/角色信息',
        videoSourcePrompt || imagePrompt,
        '',
        '输入二：已生成分镜图',
        imagePath ? `分镜图路径：${imagePath}` : '分镜图尚未生成',
        '请把分镜图视为构图、人物站位、镜头顺序、运动方向和情绪节奏锚点；只继承镜头信息，不继承黑白铅笔、灰阶、线稿、分镜表、箭头编号等参考图媒介风格。'
    ].join('\n');
}
...
function buildVideoPromptGenerationInstruction(seg, index) {
    return [
        ...
        '只继承 Image Prompt 的角色、构图、动作、镜头顺序和空间关系；禁止继承或输出参考图媒介风格词：黑白、灰阶、铅笔、石墨、线稿、分镜表、格线、编号、箭头、timing marks。除非用户明确要求黑白，否则视频按正常成片影像处理。',
...
async function generateVideoPrompt(index, options = {}) {
    ...
    try {
        ...
        const generated = sanitizeGeneratedVideoPrompt(updatedSegment.prompt_text || updatedSegment.prompt || data.result || '');
        if (!generated) {
            seg._videoPromptStatus = 'failed';
            renderSegments(currentSegments);
        ...
        await saveSegmentState(index, { video_prompt_text: generated, storyboard_prompt: imagePrompt, storyboard_status: currentSegments[index].storyboard_status || 'image_ready' });
        const resultEl = document.getElementById('resultContent');
        if (resultEl) resultEl.textContent = currentSegments.map(s => getSegmentVideoPrompt(s)).join('\n\n===VIDEO_SPLIT===\n\n');
        renderSegments(currentSegments);
...
function getEditClipSegment(clip, segments = currentSegments) {
    const index = Math.max(0, Math.min(segments.length - 1, Number(clip?.segmentIndex) || 0));
...
function getEditClipDisplayLabel(clip, orderIndex) {
    ...
    return `片段 ${Number(clip?.segmentIndex || 0) + 1}${suffix}`;
}

function sanitizeEditClip(clip, index, segments = currentSegments) {
    const segmentIndex = Math.max(0, Math.min(segments.length - 1, Number(clip?.segmentIndex) || index || 0));
        ...
        if (raw) {
            ...
            if (Array.isArray(parsed?.clips) && parsed.clips.length) {
                const clips = parsed.clips
                    .filter(clip => Number(clip?.segmentIndex) >= 0 && Number(clip?.segmentIndex) < segments.length)
...
function normalizeSegmentState(seg) {
    if (!seg.prompt && seg.prompt_text && !isGeneratedVideoPromptText(seg.prompt_text, seg)) seg.prompt = seg.prompt_text;
    if (!seg._original) {
...
async function saveSegmentState(index, extra = {}, options = {}) {
    ...
    const payload = {
        prompt_text: prompt,
        source_script: seg.source_script || seg._original || prompt,
        duration,
...
async function generateStoryboardPrompt(index, options = {}) {
    ...
    if (!seg?.id) {
        ...
        const assetScope = getScopedAssetPayload(segmentText, { segment: seg, fallbackToSelected: true });
    ...
    try {
        ...
        const assetScope = getScopedAssetPayload(`${segmentText}\n${backboneText}`, { segment: seg, fallbackToSelected: true });
        const assetTagLine = buildAssetTagLine(assetScope);
        const storyboardInputText = [
            assetTagLine ? `人物/场景标签：\n${assetTagLine}` : '',
            backboneText ? `script-backbone-compact 主干：\n${backboneText}` : '',
            `分段原文：\n${segmentText}`
        ].filter(Boolean).join('\n\n');
        const scopedInstruction = [
            ...
            '当前片段已经过 script-backbone-compact 提炼；生成 Image Prompt 时优先使用主干里的剧情动作链、角色动作链、场景、道具和情绪，分段原文只作为细节补充。',
            assetTagLine ? `最终 Image Prompt 必须原样包含这些人物/场景标签：${assetTagLine}` : '',
            `当前片段输入如下，只能根据这一段生成 Image Prompt，不要引用其他片段：\n${storyboardInputText}`
        ].join('\n\n');
        await saveSegmentState(index, {
        ...
        if (!updatedPrompt || !isTargetStoryboardPromptFormat(updatedPrompt)) {
            updatedPrompt = buildLocalStoryboardPrompt(index, `${backboneText}\n${segmentText}`.trim(), assetScope);
        }
        updatedPrompt = ensureStoryboardAssetTags(updatedPrompt, assetScope);
        currentSegments[index] = {
            ...seg,
            ...updatedSegment,
        ...
        const segmentText = getSegmentSourceText(seg);
        const backboneText = seg?._backboneCompact || '';
        const assetScope = getScopedAssetPayload(`${segmentText}\n${backboneText}`, { segment: seg, fallbackToSelected: true });
        const fallbackPrompt = segmentText ? buildLocalStoryboardPrompt(index, `${backboneText}\n${segmentText}`.trim(), assetScope) : '';
...
function getMainlineStoryboardPromptInstruction() {
    return [
        ...
        '强制采用这一类成品结构：参考图对应：【图1】角色A，【图2】角色B。人物Tag：#角色A #角色B。场景Tag：#场景名。生成单张六格动态分镜表，当前地点/场景，21:9或16:9，三乘二清晰分格，每格内部按16:9构图，角落仅保留极小黑色编号01-06；偏古风3D动画预演分镜，干净高调黑白灰阶，明亮灰白底，单色石墨线稿叠加柔和体积明暗，环境细节密集但画面干净，角色有立体体积感和可读剪影，半写实古风比例，自然五官，克制表情，不要大眼萌系或幼态卡通脸，电影感构图，前景遮挡，倾斜透视，动态调度，可见镜头路径箭头、身体重心线、情绪冲击线。',
        '必须写“根据以下剧情转化镜头，不复述台词：”后面接一句视觉剧情概括。这里禁止逐字复述对白，只把对白转换成可见动作、口型、表情、视线、站位和情绪转折。',
        '只允许这些主要段落标签：参考图对应、人物Tag、场景Tag、角色锁、背景锁、01、02、03、04、05、06、负面提示。严禁出现旧网页模板标题：固定视觉风格、分镜分格要求、原始台本依据、当前镜头内容、逐格镜头要求。',

...
function initPromptMentionInputs() {
    ['scriptInput', 'scriptProcessInstruction'].forEach(id => {
        ...
        el.addEventListener('change', refreshPromptMentionBadges);
...
function initStoryboardPromptToggle() {
    ...
    if (toggle) toggle.checked = localStorage.getItem(STORYBOARD_PROMPTS_STORAGE_KEY) === '1';
...
function updateAdvancedSettingsSummary() {
    ...
    if (storyboardSummary) storyboardSummary.textContent = storyboardEnabled ? '分镜参考：已启用' : '分镜参考：未启用';
    if (storyboardText) storyboardText.textContent = storyboardEnabled ? '已启用' : '未启用';
    if (storyboardHint) {
        storyboardHint.textContent = storyboardEnabled
            ? '开启后会先按台词时长切段，并自动生成分镜图作为视频参考。'
            : '未启用时直接生成常规视频台本；开启后会自动生成分镜图作为视频参考。';
    }
...
function isStoryboardModeEnabled() {
    ...
    return localStorage.getItem(STORYBOARD_PROMPTS_STORAGE_KEY) === '1';
}
...
function syncStoryboardPromptToggle() {
    ...
    if (btn) {
        ...
        btn.textContent = enabled ? '🙈 关闭故事板' : '🖼️ 启用故事板';
    }
    if (generateBtn) generateBtn.classList.toggle('hidden', !enabled);
    if (status) status.textContent = enabled ? '故事板已启用' : '故事板已关闭';
    updateAdvancedSettingsSummary();
}
...
function setStoryboardPromptsEnabled(enabled, options = {}) {
    ...
    showToast(
        enabled
            ? '已启用故事板功能，生成视频台本时会先按台词时长切段并自动生成分镜图'
            : '已关闭故事板功能，生成视频台本时只生成常规分镜提示词',
        'success'
    );
}
...
function fillBreakdownToStoryboard() {
    ...
    showToast('已填入分镜生成参考文本；请先在视频生成页生成视频台本，再生成故事板提示词', 'success');
}
...
async function processScript() {
    ...
    showToast('视频台本生成任务已提交，可继续操作其他内容', 'info');
    ...
    try {
        ...
        if (panelTitle) panelTitle.textContent = isStoryboardModeEnabled() ? '📋 故事板分段预览' : '📋 视频台本预览';
        ...
        const res = await fetch('/api/process-script/stream', {
            ...
            body: JSON.stringify({
                project_id: currentProject || '',
                script: script,
                scenes: getSelectedIdsInDisplayOrder('scene'),
                characters: getSelectedIdsInDisplayOrder('char'),
                director_mode_id: directorModeId,
                instruction: extraInstruction,
                storyboard_mode: isStoryboardModeEnabled(),
        ...
        // 更新片段数
        document.getElementById('segmentCount').textContent = isStoryboardModeEnabled() ? `${data.segment_count}个分镜段落` : `${data.segment_count}个片段`;

        // 渲染可编辑的台本预览
        renderSegments(currentSegments);
        autoCollapseWorkflowInput('script');

        updateInlineLiveProgress('process-script', { stage: 'done', text: data.message || '视频台本生成完成', charCount: (data.result || '').length });
        showToast(
            isStoryboardModeEnabled()
                ? `分段完成，共${data.segment_count}个分镜段落，正在后台生成分镜图`
                : `处理完成，共${data.segment_count}个视频片段，请预览并编辑视频台本`,
            'success'
        );
        if (isStoryboardModeEnabled()) {
            setTimeout(() => autoGenerateStoryboardImagesAfterScript(), 0);
        }
...
async function prepareProjectUe5JsonText() {
    if (!currentProject || !currentSegments.length) {
        showToast('请先生成视频台本后再整理导入引擎台本', 'warning');
        return;
    }
...
function importSegmentsToUe5JsonText() {
    if (!currentSegments.length) {
        showToast('请先生成视频台本，再导入到导入引擎', 'warning');
        return;
    }
...
function renderActiveSegmentDetail(seg, i) {
    ...
    return `
        <div class="segment-item segment-detail-card" id="segment-${i}">
            <div class="segment-header">
                <span class="segment-title">📹 片段 ${i + 1}</span>
                <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
                    <span class="badge" style="font-size:11px">⏱ ${duration}秒</span>
                    <span class="badge" style="font-size:11px">🎬 ${sbCount}个分镜</span>
                    ${scene ? `<span class="badge" style="font-size:11px">📍 ${escHtml(scene)}</span>` : ''}
                    ${taskText}
                    <span class="segment-status ${statusClass}">${statusLabel}</span>
                </div>
            </div>
            <div class="segment-editor mention-editor" id="segEditor-${i}" data-rows="${rows}" data-placeholder="在此编辑台本，输入 @ 引用素材或提示词库"></div>
            <div class="mention-preview-row">
                ...
                <span class="mention-preview-hint">在台本里直接打 @，可选择人物/场景素材或提示词库；人物/场景会按出现顺序作为视频参考图，提示词库会作为附加生成要求</span>
            </div>
            <details class="collapsible-panel secondary-edit-panel segment-secondary-edit-panel">
                ...
                <div class="collapsible-panel-body">
                    <div class="form-group">
                        <label>输入台本或二次处理要求</label>
                        <div class="segment-editor mention-editor segment-instruction-editor" id="segmentReprocessInstruction-${i}" data-rows="5" data-placeholder="例如：按这段新台本修正本片段；加强动作细节；@ 引用人物/场景素材或提示词库；时长控制在8秒内。"></div>
                        <div class="mention-preview-row">
                            <button type="button" class="btn btn-sm btn-secondary mention-insert-btn" onclick="openMentionPickerForReprocess(${i})">@ 插入</button>
                            <span class="mention-preview-hint">在二次处理要求里直接打 @，可选择人物/场景素材或提示词库。</span>
                        </div>
                        ...
                        <small class="field-hint">只会基于当前片段的分镜剧本重新处理，并替换本片段内容。</small>
                    </div>
                    <button id="btnReprocessSegment-${i}" class="btn btn-primary btn-lg" onclick="reprocessSingleSegment(${i})">
                        <span class="btn-icon">✏️</span> 二次处理本片段
                    </button>
                </div>
            </details>
            ${renderControlDataBlock(seg, i)}
            ${isStoryboardModeEnabled() ? renderStoryboardBlock(seg, i) : ''}
            <div class="segment-ref-panel">
                ...
                <div id="segRefWrap-${i}" class="${mode==='text_to_video'?'hidden':''}">
                    <div class="segment-ref-title">本片段参考图</div>
                    <div class="segment-ref-hint">这一段视频会用到的参考图，由上方台本里 @ 到的人物/场景自动决定。如果想再加一张图（比如手机拍的截图、临时的风格参考），可以在下面上传，它会和台本里 @ 到的素材一起发给视频模型。</div>
                    <div class="segment-upload-row">
                        <span>本片段临时上传参考图：</span>
            ...
            <div class="segment-actions">
                <button class="btn btn-sm btn-primary" onclick="generateSingleVideo(${i})">🚀 生成本片段</button>
                <button class="btn btn-sm btn-primary" onclick="generateSingleVideoFromPrevious(${i})">🔗 根据上条视频生成</button>
                <button class="btn btn-sm btn-danger" onclick="deleteSegment(${i})">🗑 删除</button>
            </div>
            ${videoSrc ? `<div class="segment-video-result"><video src="${escAttr(videoSrc)}" controls preload="none"></video>${seg._localVideoPath ? `<div class="local-path">已保存到资料库：${escHtml(seg._localVideoPath)}</div>` : ''}</div>` : ''}
        </div>`;
}
...
function initActiveSegmentEditors(seg, i) {
    ...
    editor.setValue(seg.prompt || seg.prompt_text || '', { legacyRefImages });
...
async function reprocessSingleSegment(index) {
    ...
    const editor = document.getElementById(`segEditor-${index}`);
    const currentScript = (editor ? readMentionEditorValue(editor) : (seg.prompt || seg.prompt_text || '')).trim();
...
function renderStoryboardBlock(seg, i) {
    ...
    const promptTools = `
        <details class="collapsible-panel secondary-edit-panel segment-secondary-edit-panel" ${failed || !hasPrompt ? 'open' : ''}>
            ...
            <div class="collapsible-panel-body">
                <textarea class="storyboard-prompt-editor" id="storyboardPrompt-${i}" rows="6" placeholder="当前片段的分镜图生成提示词" onblur="saveStoryboardPromptEdit(${i})">${escHtml(prompt)}</textarea>
                <div class="storyboard-actions">
                    <button class="btn btn-sm btn-secondary" onclick="generateStoryboardPrompt(${i})">重新生成提示词</button>
                    <button class="btn btn-sm btn-secondary" onclick="regenerateStoryboardImage(${i})">按提示词重新生成图</button>
                </div>
            </div>
        </details>`;
    const retryActions = `
        <div class="storyboard-actions">
            <button class="btn btn-sm btn-primary" onclick="regenerateStoryboardImage(${i})">${hasPrompt ? '重新生成分镜图' : '生成提示词并出图'}</button>
            <button class="btn btn-sm btn-secondary" onclick="regenerateStoryboardImage(${i}, { regeneratePrompt: true })">重新生成提示词并出图</button>
        </div>`;
    return `
        <div class="storyboard-block ${imagePath ? 'has-image' : ''}">
            <div class="storyboard-block-header">
                <div>
                    <div class="storyboard-title">🖼️ 编辑分镜图</div>
                    <div class="storyboard-hint">输入编辑提示后，系统会把当前分镜图和提示词一起上传，重新生成并覆盖本片段故事板</div>
                </div>
            ...
            ${imagePath ? `
                <div class="storyboard-preview">
                    ...
                    <div class="local-path">当前分镜图：${escHtml(imagePath)}。生成视频时会作为自动附加参考图输入。</div>
                </div>
                <div class="storyboard-edit-box">
                    ...
                    <div class="field-hint">编辑时会把当前故事板图作为第一张参考图，并把这段编辑提示词一起提交；成功后自动覆盖当前分镜图。</div>
                </div>` : ''}
            ${!imagePath ? `
                <div class="storyboard-edit-box">
                    <div class="field-hint">${failed ? '分镜图生成失败，可以在这里直接重新生成。' : '当前片段还没有可编辑的分镜图。故事板模式会在生成视频台本后自动出图；如果自动生成失败，可在这里重新生成。'}</div>
                    ${retryActions}
                </div>` : ''}
            ${promptTools}
...
function normalizeSegmentState(seg) {
    ...
    if (!seg._original) seg._original = seg.prompt || seg.prompt_text || '';
...
async function deleteStoryboardFrame(index, framePosition = 'start') {
    ...
    showToast('故事板图片已删除，不会再作为视频参考图', 'success');
}
...
function getVideoStoryboardReferenceLabels(seg) {
    const labels = [];
    if (seg?.storyboard_image_path && isStoryboardModeEnabled()) labels.push('故事板图');
    if (seg?.end_storyboard_image_path && isStoryboardModeEnabled()) labels.push('尾帧故事板图');
    return labels;
}
...
function splitSegment(index) {
    ...
    document.getElementById('segmentCount').textContent = `${currentSegments.length}个片段`;
    renderSegments(currentSegments);
    renderStoryboardOnlyList(currentSegments);
...
async function processStoryboardOnly() {
    ...
    if (!currentProject || !currentSegments.length) {
        ...
        showToast('请先在“视频生成”页使用编导提示词生成视频台本片段，再点击“生成分镜提示词”回填故事板文本框', 'warning');
        switchToTab('script');
        return;
    }
    const btn = document.getElementById('btnStoryboardProcess');
    const resetBtn = setButtonBusy(btn, '<span class="btn-icon">⏳</span> 生成中，可继续操作');
    startInlineFakeProgress('storyboard-only-process', btn, '正在根据原始台本生成故事板提示词', 120);
    showToast('将优先使用原始台本，为当前片段生成故事板提示词并自动回填', 'info');
    setWorkflowResultEmpty('storyboard');
    try {
        const fixedInstruction = '如果提供原始台本，优先按原始台本的剧情、对白、动作和空间关系生成故事板；当前项目片段只用于确定片段顺序、边界、时长和镜头数量。不要重新改写视频台本，不要重新拆成新的项目；只按现有片段顺序为每个片段生成一张分格故事板大图提示词。故事板目的不是画漂亮插画，而是保持人物站位和镜头正确。每个分镜/镜头单独占一个格子，多个格子拼接成一张大图，按镜头顺序从左到右、从上到下排列；不要生成首帧或尾帧，不要只画一个核心瞬间；必须按每个格子写清谁在谁左边/右边，谁在谁前方/后方，谁更靠近镜头，谁面向谁，谁看向谁，镜头景别、机位高度、拍摄方向和画面轴线；没有明确位移时保持上一格的人物左右关系和前后层次；并明确要求故事板大图内无文字、每个格子内无文字、无字幕、无对白气泡、无镜头编号、无UI、无水印。';
        const data = await submitStoryboardPromptJob(fixedInstruction, 'storyboard-only-process');
        if (!data) return;
        currentSegments = data.segments || [];
        ...
        currentProjectInfo = projectsCache.find(p => p.id === currentProject) || currentProjectInfo;
        updateCurrentProjectBar();
        showToast(`已为 ${currentSegments.length} 个视频片段生成并回填故事板提示词`, 'success');
    } catch (e) {
...
async function generateStoryboardPrompt(index, options = {}) {
    const seg = currentSegments[index];
    if (!seg?.id) {
        showToast('请先生成视频台本后再生成分镜提示词', 'warning');
        return false;
    }
    const editor = document.getElementById(`segEditor-${index}`);
    if (editor) {
    ...
    const frameLabel = '故事板';
    const oldStoryboardPrompt = seg.storyboard_prompt || '';
    seg.storyboard_prompt = '';
    seg.storyboard_status = 'generating';
    seg.continuity_notes = '正在重新生成故事板提示词，不会复用当前输入框中的旧提示词。';
    renderSegments(currentSegments);
    renderStoryboardOnlyList(currentSegments);
    if (!options.silentLoading) startInlineFakeProgress(`storyboard-prompt-${framePosition}-${index}`, document.querySelector(`#storyboardPrompt-${index}`)?.closest('.storyboard-block'), `正在生成片段 ${index + 1} ${frameLabel}提示词`, 60);
    showToast(`片段 ${index + 1} ${frameLabel}提示词生成任务已提交，可继续操作`, 'info');
    try {
        const res = await fetch(`/api/segments/${seg.id}/storyboard-prompt`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ratio: getStoryboardRatio(), frame_position: framePosition, regenerate: true })
        });
        const data = await res.json();
        if (!res.ok) {
            seg.storyboard_prompt = oldStoryboardPrompt;
            seg.storyboard_status = oldStoryboardPrompt ? 'prompt_ready' : 'failed';
            seg.continuity_notes = data.error || '分镜提示词生成失败';
            renderSegments(currentSegments);
            renderStoryboardOnlyList(currentSegments);
            showToast(data.error || '分镜提示词生成失败', 'error');
            return false;
        }
        currentSegments[index] = { ...seg, ...data, prompt: data.prompt_text || seg.prompt };
        renderSegments(currentSegments);
        renderStoryboardOnlyList(currentSegments);
        showToast(`片段 ${index + 1} ${frameLabel}提示词已生成`, 'success');
        return true;
    } catch (e) {
        showToast('分镜提示词请求失败: ' + e.message, 'error');
        return false;
    } finally {
...
async function generateAllStoryboardPrompts() {
    ...
    const btn = document.querySelector('[onclick="generateAllStoryboardPrompts()"]');
    const resetBtn = setButtonBusy(btn, '⏳ 生成中');
    startInlineFakeProgress('all-storyboard-prompts', btn, '正在整体生成所有故事板提示词', 180);
    showToast('已开始按原始台本整体生成故事板提示词，请等待完成后自动回填', 'info');
    try {
        const fixedInstruction = '需要同时生成所有片段的分格故事板大图提示词。如果提供原始台本，优先按原始台本的剧情、对白、动作和空间关系生成故事板；当前项目片段只用于确定片段顺序、边界、时长和镜头数量。故事板目的不是画漂亮插画，而是让AI理解并保持人物空间站位和镜头方向。每个分镜/镜头单独占一个格子，多个格子拼接成一张大图，按镜头顺序从左到右、从上到下排列。不要生成首帧或尾帧，不要只生成一个核心静态画面。提示词重点按格子写清：谁在谁左边/右边，谁在谁前方/后方，谁更靠近镜头，谁在前景/中景/背景，谁面向谁或侧向谁，视线落点；同时写清镜头景别、机位高度、拍摄方向、画面轴线和固定参照物位置。没有明确位移时保持上一格的人物左右关系和前后层次。人物动作简单描写即可，并明确要求故事板大图内无文字、每个格子内无文字、无字幕、无对白气泡、无镜头编号、无UI、无水印。';
        const data = await submitStoryboardPromptJob(fixedInstruction, 'all-storyboard-prompts');
        if (!data) return;
        currentSegments = data.segments || currentSegments;
        currentSegments.forEach(normalizeSegmentState);
        renderSegments(currentSegments);
        renderStoryboardOnlyList(currentSegments);
        showToast(`已生成 ${data.segment_count || currentSegments.length} 个分镜的故事板提示词`, 'success');
    } catch (e) {
        showToast('全部故事板提示词请求失败: ' + e.message, 'error');
    } finally {
...
async function submitStoryboardPromptJob(fixedInstruction, progressKey) {
    ...
    const res = await fetch(`/api/projects/${currentProject}/storyboard-prompts`, {
        ...
        body: JSON.stringify({
            ...
            fixed_instruction: fixedInstruction,
    ...
    if (!res.ok) {
        showToast(submitData.error || '故事板提示词任务提交失败', 'error');
        return null;
    }
...
async function pollStoryboardPromptJob(jobId, progressKey) {
    ...
    while (Date.now() - startedAt < maxMs) {
        ...
        const data = await res.json();
        if (!res.ok) {
            showToast(data.error || '故事板提示词状态查询失败', 'error');
            return null;
        }
        if (progressKey) {
            const progress = Math.min(90, 15 + attempts * 3);
            updateInlineFakeProgress(progressKey, progress, data.message || '正在生成故事板提示词');
        }
        if (data.status === 'completed') return data;
        if (data.status === 'failed') {
            showToast(data.error || data.message || '故事板提示词生成失败', 'error');
            return null;
        }
    }
    showToast('故事板提示词生成轮询超时，请稍后刷新项目查看结果', 'warning');
    return null;
}
...
async function saveStoryboardPromptEdit(index, framePosition = 'start') {
    ...
    showToast(`片段 ${index + 1} 故事板提示词已保存`, 'success');
}
...
async function generateStoryboardImage(index, options = {}) {
    const seg = currentSegments[index];
    if (!seg?.id) {
        showToast('请先生成视频台本后再生成分镜图', 'warning');
        return false;
    }
    const framePosition = 'start';
    const isEnd = false;
    const frameLabel = '故事板图';
    const promptEl = document.getElementById(`storyboardPrompt-${index}`);
    const prompt = (promptEl?.value || seg.storyboard_prompt || '').trim();
    if (!prompt) {
        showToast('请先生成或填写故事板图提示词', 'warning');
        return false;
    }
    seg.storyboard_prompt = prompt;
    seg.storyboard_status = 'generating';
    ...
    renderSegments(currentSegments);
    renderStoryboardOnlyList(currentSegments);
    if (!options.silentLoading) startInlineFakeProgress(`storyboard-image-${framePosition}-${index}`, document.querySelector(`#storyboardPrompt-${index}`)?.closest('.storyboard-block'), `正在生成片段 ${index + 1} ${frameLabel}`, 180);
    ...
    try {
        ...
        if (!res.ok) {
            ...
            seg.continuity_notes = data.error || '分镜图生成失败';
            showToast(data.error || '分镜图生成失败', 'error');
            renderSegments(currentSegments);
            renderStoryboardOnlyList(currentSegments);
            return false;
        }
...
async function editStoryboardImage(index) {
    const seg = currentSegments[index];
    if (!seg?.id) {
        showToast('请先生成视频台本后再编辑分镜图', 'warning');
        return;
    }
    if (!seg.storyboard_image_path) {
        showToast('请先生成故事板图后再编辑', 'warning');
        return;
    }
    ...
    if (!editPrompt) {
        showToast('请填写故事板图编辑提示词', 'warning');
        return;
    }

    seg._storyboardEditPrompt = editPrompt;
    seg.storyboard_status = 'generating';
    seg.continuity_notes = '正在按编辑提示重新生成故事板图，完成后会覆盖当前故事板图。';
    renderSegments(currentSegments);
    renderStoryboardOnlyList(currentSegments);
    startInlineFakeProgress(`storyboard-edit-${index}`, document.querySelector(`#storyboardPrompt-${index}`)?.closest('.storyboard-block'), `正在编辑片段 ${index + 1} 故事板图`, 180);
    showToast(`片段 ${index + 1} 故事板图编辑任务已提交，可继续操作`, 'info');
    ...
    try {
        ...
        if (!res.ok) {
            ...
            seg.continuity_notes = data.error || '故事板图编辑失败';
            showToast(data.error || '故事板图编辑失败', 'error');
            renderSegments(currentSegments);
            renderStoryboardOnlyList(currentSegments);
            return;
        }
        ...
        renderSegments(currentSegments);
        renderStoryboardOnlyList(currentSegments);
        showToast(`片段 ${index + 1} 故事板图已编辑并覆盖`, 'success');
    } catch (e) {
        seg.storyboard_status = 'failed';
        seg.continuity_notes = '故事板图编辑请求失败: ' + e.message;
        renderSegments(currentSegments);
        renderStoryboardOnlyList(currentSegments);
        showToast('故事板图编辑请求失败: ' + e.message, 'error');
    } finally {
...
async function autoGenerateStoryboardImagesAfterScript() {
    if (!isStoryboardModeEnabled() || !currentSegments.length) return;
    const targets = currentSegments
        .map((seg, index) => ({ seg, index }))
        .filter(item => item.seg?.id && !item.seg.storyboard_image_path);
    if (!targets.length) return;

    showToast(`视频台本已显示，正在后台生成 ${targets.length} 张分镜图`, 'info');
    const results = await runStoryboardImageAutoQueue(targets, { concurrency: targets.length });
    const okCount = results.filter(Boolean).length;
    if (okCount === targets.length) {
        showToast(`分镜图已全部生成 ${okCount}/${targets.length}`, 'success');
    } else {
        showToast(`分镜图生成完成 ${okCount}/${targets.length}，失败的片段可在卡片内重新生成`, okCount ? 'warning' : 'error');
    }
}
...
async function generateSingleVideo(index, options = {}) {
    ...
    const editor = document.getElementById(`segEditor-${index}`);
    let prompt = editor ? readMentionEditorValue(editor).trim() : seg.prompt;
    ...
    const mode = document.getElementById(`segMode-${index}`)?.value || seg._mode || 'text_to_video';
    ...
    const storyboardPromptEl = document.getElementById(`storyboardPrompt-${index}`);
    if (storyboardPromptEl?.value?.trim()) {
        seg.storyboard_prompt = storyboardPromptEl.value.trim();
    }
    seg.end_storyboard_prompt = '';
    seg.end_storyboard_status = '';
...
function getWorkbenchStats(project, segments = []) {
    ...
    if (hasProject && !totalSegments) {
        nextStep = {
            key: 'script',
            title: hasScript ? '生成视频台本' : '输入剧本',
            description: '进入视频生成页，补全剧本、素材和模式后生成可编辑片段。',
            tab: 'script',
            actionLabel: '开始视频生成'
        };
        ...
        nextStep = {
            key: 'video',
            title: '生成视频片段',
            description: `已完成 ${completedVideos}/${totalSegments} 个视频片段。分镜可作为参考辅助，但不会阻塞生成视频。`,
            tab: 'script',
            actionLabel: '生成视频'
        };
...
function getWorkbenchFlowSteps(stats) {
    ...
    return [
        ...
        { key: 'script', label: '剧本', desc: '输入与拆解', tab: 'script', state: hasSegments ? 'done' : (stats.hasProject ? 'current' : 'pending'), lane: 'main' },
        { key: 'video', label: '视频', desc: '片段生成', tab: 'script', state: videosDone ? 'done' : (hasSegments ? 'current' : 'pending'), lane: 'main' },
        { key: 'merge', label: '成片', desc: '拼接编辑', tab: 'video-editor', state: videosDone ? 'current' : 'pending', lane: 'main' },
        { key: 'storyboard', label: '分镜', desc: '稳定构图', tab: 'storyboard', state: storyboardsDone ? 'done' : 'optional', optional: true, lane: 'optional' },
...
function openDraftProject(project) {
    ...
    document.getElementById('segmentCount').textContent = '0个片段';
